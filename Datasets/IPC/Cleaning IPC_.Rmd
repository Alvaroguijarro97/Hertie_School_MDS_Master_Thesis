...
joined_dataset <- reduce(list(CPI_model, year_to_date_var_model, yearly_var_model, montly_var_model), inner_join, by = c("city", "date"))

# Check the resulting joined dataset
head(joined_dataset)
```
```{r}
# Extract the minimum and maximum dates from the dataset
min_date <- min(joined_dataset$date)
max_date <- max(joined_dataset$date)
```

```{r}
# Create the filename using the min and max dates
filename_CPI_total <- paste0("CPI_Information_cleaned_", min_date, "-", max_date, ".xlsx")
# Write the dataframe to an Excel file
write_xlsx(joined_dataset, filename_CPI_total)
```
