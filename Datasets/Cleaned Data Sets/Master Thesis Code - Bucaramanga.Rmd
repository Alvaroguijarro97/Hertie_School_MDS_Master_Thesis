...
Let's proceed to work on the prediction for the city of BUCARAMANGA A.M.
```{r}
bucaramanga_dataset <- employed_data %>%
  filter( employed_data$city == "BUCARAMANGA A.M.") %>%
...
```{r}
bucaramanga_dataset$date <- as.Date(paste0(bucaramanga_dataset$date, "-01"), format = "%Y-%m-%d")
plot(bucaramanga_dataset$date, bucaramanga_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
buc_basic_ols <- lm(workers.geih ~ . - date, data = bucaramanga_dataset)
summary(buc_basic_ols)
...
bucaramanga_dataset_subset <- bucaramanga_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(bucaramanga_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(bucaramanga_dataset_subset$workers.geih)
qqline(bucaramanga_dataset_subset$workers.geih, col = "red")
shapiro.test(bucaramanga_dataset_subset$workers.geih)
summary(bucaramanga_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = bucaramanga_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(bucaramanga_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
bucaramanga_dataset_subset$date <- as.Date(paste0(bucaramanga_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_buc <- bucaramanga_dataset_subset[bucaramanga_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_buc <- bucaramanga_dataset_subset[bucaramanga_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_buc <- lm(workers.geih ~ . - date, data = train_data_buc)

# Summarize the model to look at coefficients
summary(model_buc)

```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 0.9814 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_buc <- lm(workers.geih ~ . - date, data = train_data_buc)
stepwise_model_buc <- step(model_buc, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_buc)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_buc$predicted_workers <- predict(stepwise_model_buc, newdata = test_data_buc)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(bucaramanga_dataset_subset$workers.geih, quantile(test_data_buc$predicted_workers,0.25)))

# Plot the actual data with the adjusted y limits
plot(bucaramanga_dataset_subset$date, bucaramanga_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(bucaramanga_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_buc$date, test_data_buc$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_buc))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_buc), resid(stepwise_model_buc), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_buc)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_buc)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_buc$predicted_workers - test_data_buc$workers.geih))
rmse <- sqrt(mean((test_data_buc$predicted_workers - test_data_buc$workers.geih)^2))
...
W = 0.9157, p-value = 0.01819: The p-value is smaller than 0.05, so we reject the null hypothesis of normality. This indicates that the residuals of the model may be not considered normally distributed, which doesn't satisfy done of the OLS assumptions.
...
DW = 2.732, p-value = 0.1383: A Durbin-Watson statistic close to 2 suggests that there is no autocorrelation in the residuals, which meets the assumption of independence. However, the p-value here is above 0.05, indicating there is no significant evidence of autocorrelation. 
...
MAE: 386166.8  and RMSE: 599713.9  suggest that the model's predictions deviate from the actual values by a large margin. Lower values of MAE and RMSE would indicate better predictive accuracy. The large values here indicate the model may not predict the target variable very well.
...
```{r}
coef_summary <- summary(stepwise_model_buc)$coefficients
...
stepwise_model_buc_sig <- lm(significant_formula, data = train_data_buc)

# Summary of the new model
summary(stepwise_model_buc_sig)

```
```{r}
# Predicting with the stepwise model
test_data_buc$predicted_workers_sig <- predict(stepwise_model_buc_sig, newdata = test_data_buc)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(bucaramanga_dataset_subset$workers.geih, mean(test_data_buc$predicted_workers)))

# Plot the actual data for the complete dataset
plot(bucaramanga_dataset_subset$date, bucaramanga_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_buc$date, test_data_buc$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_buc$date, test_data_buc$predicted_workers_sig, col='gold')
...
xlim <- range(bucaramanga_dataset_subset$date)
ylim <- range(c(bucaramanga_dataset_subset$workers.geih, test_data_buc$predicted_workers, test_data_buc$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_buc_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_buc_sig), resid(stepwise_model_buc_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_buc_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_buc_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_buc$predicted_workers_sig - test_data_buc$workers.geih))
rmse_sig <- sqrt(mean((test_data_buc$predicted_workers_sig - test_data_buc$workers.geih)^2))
...
arima_model_d<- auto.arima(workers_ts, seasonal = FALSE) #setting the order of first-differencing to 1 to see if this makes a difference

summary(arima_model)
print("")
summary(arima_model_d)
```
*ACF*: The ACF plot shows the autocorrelation of the series with its lags. Ideally, for a time series that has no autocorrelation, we would expect to see the plot drop to near-zero quickly. In this case the ACF plot shows that most of the autocorrelations are within the confidence interval (the blue dotted lines) only after the fourth lag, which is an indication that there may not be significant autocorrelation in the differenced series.
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_buc$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_buc$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_buc$workers.geih))

# Define the time series plot range
time_range <- range(bucaramanga_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(bucaramanga_dataset_subset$workers.geih, quantile(test_data_buc$predicted_workers,0.25), test_data_buc$predicted_workers_sig, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(bucaramanga_dataset_subset$date, bucaramanga_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in BUCARAMANGA A.M.")
...
mae_arima <- mean(abs(test_data_buc$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_buc$workers.geih - forecasts$mean)^2))
...
summary(forecasts_d)
```
We can visually confirm that the ARIMA model is predicting the variables better than the prior 2 models but the ARIMA model with the d argument manually set to 1 is almost producing a straigh line. Let's proceed to store the evaluation results of each to be able to better compare them at the end.
```{r}
...
ols_mae <- mean(abs(test_data_buc$predicted_workers - test_data_buc$workers.geih))
ols_rmse <- sqrt(mean((test_data_buc$predicted_workers - test_data_buc$workers.geih)^2))
ols_summary <- summary(stepwise_model_buc)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 8.951e-07, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_buc),
  AICc = AIC(stepwise_model_buc) + (2 * length(coef(stepwise_model_buc)) * (length(coef(stepwise_model_buc)) + 1)) / (length(resid(stepwise_model_buc)) - length(coef(stepwise_model_buc)) - 1),
  BIC = BIC(stepwise_model_buc)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_buc$predicted_workers_sig - test_data_buc$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_buc$predicted_workers_sig - test_data_buc$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_buc_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 1.124e-08, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_buc_sig),
  AICc = AIC(stepwise_model_buc_sig) + (2 * length(coef(stepwise_model_buc_sig)) * (length(coef(stepwise_model_buc_sig)) + 1)) / (length(resid(stepwise_model_buc_sig)) - length(coef(stepwise_model_buc_sig)) - 1),
  BIC = BIC(stepwise_model_buc_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_buc$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_buc$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_buc$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_buc$workers.geih - forecasts_d$mean)^2))
...
# Calculate residuals
residuals <- test_data_buc$workers.geih - forecasts$mean
...
*Residuals*: Use to visually assess the randomness and distribution of the residuals in our prediction. The plot of the residuals of the ARIMA model appears to exhibit some pattern, as indicated by the trend upwards. This suggests that the model may not be fully capturing the underlying data structure, which might be due to seasonality or another form of autocorrelation not addressed by the model.
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_buc$workers.geih))
```
...
workers_range <- range(bucaramanga_dataset_subset$workers.geih)

plot(bucaramanga_dataset_subset$date, bucaramanga_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in BUCARAMANGA A.M.")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_buc$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_buc$date, test_data_buc$predicted_workers, col='red', lwd=1.8)
lines(test_data_buc$date, test_data_buc$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(test_data_buc$date)), col="black", lwd=1.8, lty=2)
...
sarima_mae <- mean(abs(test_data_buc$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_buc$workers.geih - sarima_forecasts$mean)^2))
...
```{r Random forrest}
# Load data
data_rf <- bucaramanga_dataset_subset
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_buc)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_buc_sig)
...
write_xlsx(model_summaries, path = "bucaramanga_results.xlsx")
...
png("bucaramanga_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
