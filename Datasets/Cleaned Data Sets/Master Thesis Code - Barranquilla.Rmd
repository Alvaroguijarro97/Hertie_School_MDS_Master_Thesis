...
Let's proceed to work on the prediction for the city of BARRANQUILLA A.M
```{r}
barranquilla_dataset <- employed_data %>%
  filter( employed_data$city == "BARRANQUILLA A.M.") %>%
  select(-city, -eco_activity, -Category.sfp, -lat.ll, -lat_z.ll, -lng.ll, -lng_z.ll)
barranquilla_dataset
```
Let's see again how does our variable of interest looks like: 
```{r}
barranquilla_dataset$date <- as.Date(paste0(barranquilla_dataset$date, "-01"), format = "%Y-%m-%d")
plot(barranquilla_dataset$date, barranquilla_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
baq_basic_ols <- lm(workers.geih ~ . - date, data = barranquilla_dataset)
...
barranquilla_dataset_subset <- barranquilla_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(barranquilla_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(barranquilla_dataset_subset$workers.geih)
qqline(barranquilla_dataset_subset$workers.geih, col = "red")
shapiro.test(barranquilla_dataset_subset$workers.geih)
summary(barranquilla_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = barranquilla_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(barranquilla_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
barranquilla_dataset_subset$date <- as.Date(paste0(barranquilla_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_baq <- barranquilla_dataset_subset[barranquilla_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_baq <- barranquilla_dataset_subset[barranquilla_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_baq <- lm(workers.geih ~ . - date, data = train_data_baq)

# Summarize the model to look at coefficients
summary(model_baq)

```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 0.9708 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_baq <- lm(workers.geih ~ . - date, data = train_data_baq)
stepwise_model_baq <- step(model_baq, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_baq)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_baq$predicted_workers <- predict(stepwise_model_baq, newdata = test_data_baq)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(barranquilla_dataset_subset$workers.geih, mean(test_data_baq$predicted_workers)))

# Plot the actual data with the adjusted y limits
plot(barranquilla_dataset_subset$date, barranquilla_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(barranquilla_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_baq$date, test_data_baq$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_baq))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_baq), resid(stepwise_model_baq), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_baq)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_baq)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_baq$predicted_workers - test_data_baq$workers.geih))
rmse <- sqrt(mean((test_data_baq$predicted_workers - test_data_baq$workers.geih)^2))
...
W = 0.95447, p-value = 0.2071: The p-value is greater than 0.05, so we do not reject the null hypothesis of normality. This indicates that the residuals of the model may be considered normally distributed, which satisfies one of the OLS assumptions.
...
MAE: 219575.1 and RMSE: 302015.3 suggest that the model's predictions deviate from the actual values by a large margin. Lower values of MAE and RMSE would indicate better predictive accuracy. The large values here indicate the model may not predict the target variable very well.
...
```{r}
coef_summary <- summary(stepwise_model_baq)$coefficients
...
stepwise_model_baq_sig <- lm(significant_formula, data = train_data_baq)

# Summary of the new model
summary(stepwise_model_baq_sig)

```
```{r}
# Predicting with the stepwise model
test_data_baq$predicted_workers_sig <- predict(stepwise_model_baq_sig, newdata = test_data_baq)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(barranquilla_dataset_subset$workers.geih, mean(test_data_baq$predicted_workers)))

# Plot the actual data for the complete dataset
plot(barranquilla_dataset_subset$date, barranquilla_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_baq$date, test_data_baq$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_baq$date, test_data_baq$predicted_workers_sig, col='gold')
...
xlim <- range(barranquilla_dataset_subset$date)
ylim <- range(c(barranquilla_dataset_subset$workers.geih, test_data_baq$predicted_workers, test_data_baq$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_baq_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_baq_sig), resid(stepwise_model_baq_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_baq_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_baq_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_baq$predicted_workers_sig - test_data_baq$workers.geih))
rmse_sig <- sqrt(mean((test_data_baq$predicted_workers_sig - test_data_baq$workers.geih)^2))
...
workers_ts <- ts(train_data_baq$workers.geih, frequency = 12)
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_baq$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_baq$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_baq$workers.geih))

# Define the time series plot range
time_range <- range(barranquilla_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(barranquilla_dataset_subset$workers.geih, quantile(test_data_baq$predicted_workers,0.25), test_data_baq$predicted_workers_sig, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(barranquilla_dataset_subset$date, barranquilla_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in BARRANQUILLA A.M.")
...
mae_arima <- mean(abs(test_data_baq$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_baq$workers.geih - forecasts$mean)^2))
...
ols_mae <- mean(abs(test_data_baq$predicted_workers - test_data_baq$workers.geih))
ols_rmse <- sqrt(mean((test_data_baq$predicted_workers - test_data_baq$workers.geih)^2))
ols_summary <- summary(stepwise_model_baq)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 5.165e-09, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_baq),
  AICc = AIC(stepwise_model_baq) + (2 * length(coef(stepwise_model_baq)) * (length(coef(stepwise_model_baq)) + 1)) / (length(resid(stepwise_model_baq)) - length(coef(stepwise_model_baq)) - 1),
  BIC = BIC(stepwise_model_baq)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_baq$predicted_workers_sig - test_data_baq$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_baq$predicted_workers_sig - test_data_baq$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_baq_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 3.526e-10, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_baq_sig),
  AICc = AIC(stepwise_model_baq_sig) + (2 * length(coef(stepwise_model_baq_sig)) * (length(coef(stepwise_model_baq_sig)) + 1)) / (length(resid(stepwise_model_baq_sig)) - length(coef(stepwise_model_baq_sig)) - 1),
  BIC = BIC(stepwise_model_baq_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_baq$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_baq$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_baq$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_baq$workers.geih - forecasts_d$mean)^2))
...
# Calculate residuals
residuals <- test_data_baq$workers.geih - forecasts$mean
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_baq$workers.geih))
```
...
workers_range <- range(barranquilla_dataset_subset$workers.geih, 
                       quantile(test_data_baq$predicted_workers,0.25), 
                       test_data_baq$predicted_workers_sig, 
                       all_confidence_intervals)

plot(barranquilla_dataset_subset$date, barranquilla_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in BARRANQUILLA A.M.")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_baq$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_baq$date, test_data_baq$predicted_workers, col='red', lwd=1.8)
lines(test_data_baq$date, test_data_baq$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(test_data_baq$date)), col="black", lwd=1.8, lty=2)
...
sarima_mae <- mean(abs(test_data_baq$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_baq$workers.geih - sarima_forecasts$mean)^2))
...
```{r Random forrest}
# Load data
data_rf <- barranquilla_dataset_subset
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_baq)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_baq_sig)
...
write_xlsx(model_summaries, path = "barranquilla_results.xlsx")

```

```{r export plot}
# Open a PNG graphics device
png("barranquilla_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
