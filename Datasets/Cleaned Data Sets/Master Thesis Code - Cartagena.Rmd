...
Let's proceed to work on the prediction for the city of CARTAGENA
```{r}
cartagena_dataset <- employed_data %>%
  filter( employed_data$city == "CARTAGENA") %>%
...
```{r}
cartagena_dataset$date <- as.Date(paste0(cartagena_dataset$date, "-01"), format = "%Y-%m-%d")
plot(cartagena_dataset$date, cartagena_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
ctg_basic_ols <- lm(workers.geih ~ . - date, data = cartagena_dataset)
summary(ctg_basic_ols)
...
cartagena_dataset_subset <- cartagena_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(cartagena_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(cartagena_dataset_subset$workers.geih)
qqline(cartagena_dataset_subset$workers.geih, col = "red")
shapiro.test(cartagena_dataset_subset$workers.geih)
summary(cartagena_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = cartagena_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(cartagena_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
cartagena_dataset_subset$date <- as.Date(paste0(cartagena_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_ctg <- cartagena_dataset_subset[cartagena_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_ctg <- cartagena_dataset_subset[cartagena_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_ctg <- lm(workers.geih ~ . - date, data = train_data_ctg)

# Summarize the model to look at coefficients
summary(model_ctg)

```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 0.9795 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_ctg <- lm(workers.geih ~ . - date, data = train_data_ctg)
stepwise_model_ctg <- step(model_ctg, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_ctg)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_ctg$predicted_workers <- predict(stepwise_model_ctg, newdata = test_data_ctg)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cartagena_dataset_subset$workers.geih, mean(test_data_ctg$predicted_workers)))

# Plot the actual data with the adjusted y limits
plot(cartagena_dataset_subset$date, cartagena_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(cartagena_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_ctg$date, test_data_ctg$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_ctg))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_ctg), resid(stepwise_model_ctg), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_ctg)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_ctg)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_ctg$predicted_workers - test_data_ctg$workers.geih))
rmse <- sqrt(mean((test_data_ctg$predicted_workers - test_data_ctg$workers.geih)^2))
...
W = 0.95543, p-value = 0.2199: The p-value is greater than the standard alpha level of 0.05, suggesting that there's no strong evidence against the null hypothesis of normality in the residuals. This implies that the residuals from the model may be normally distributed, aligning with the assumptions necessary for OLS regression.
*Homoscedasticity (Plot of Residuals vs. Fitted Values)*
The plot exhibits residuals scattered somewhat evenly above and below the zero line without any apparent pattern, such as a funnel shape or trend, which would indicate heteroscedasticity. This lack of pattern suggests that the variance of the residuals is constant and that the homoscedasticity assumption is likely met.
*Independence (Durbin-Watson test)*
DW = 2.2991, p-value = 0.01744: A DW value around 2 suggests the residuals are uncorrelated, indicating independence. However, the p-value below 0.05 suggests some evidence of positive autocorrelation. This contradiction warrants a closer examination of the data and potentially additional testing or model adjustments.
*Multicollinearity (VIF)*
Several variables show very high VIF values, suggesting that they are highly correlated with one another. Multicollinearity can affect the stability of the coefficient estimates and might lead to less reliable statistical inferences, which means that care should be taken when interpreting the model's coefficients.
*Prediction Performance (MAE and RMSE)*
MAE: 35075.15 and RMSE: 39117.27: These performance metrics indicate the average absolute error and the standard deviation of the residuals, respectively. 
...
```{r}
coef_summary <- summary(stepwise_model_ctg)$coefficients
...
stepwise_model_ctg_sig <- lm(significant_formula, data = train_data_ctg)

# Summary of the new model
summary(stepwise_model_ctg_sig)
```

```{r}
# Predicting with the stepwise model
test_data_ctg$predicted_workers_sig <- predict(stepwise_model_ctg_sig, newdata = test_data_ctg)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cartagena_dataset_subset$workers.geih, mean(test_data_ctg$predicted_workers)))

# Plot the actual data for the complete dataset
plot(cartagena_dataset_subset$date, cartagena_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_ctg$date, test_data_ctg$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_ctg$date, test_data_ctg$predicted_workers_sig, col='gold')
...
xlim <- range(cartagena_dataset_subset$date)
ylim <- range(c(cartagena_dataset_subset$workers.geih, test_data_ctg$predicted_workers, test_data_ctg$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_ctg_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_ctg_sig), resid(stepwise_model_ctg_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_ctg_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_ctg_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_ctg$predicted_workers_sig - test_data_ctg$workers.geih))
rmse_sig <- sqrt(mean((test_data_ctg$predicted_workers_sig - test_data_ctg$workers.geih)^2))
...
workers_ts <- ts(train_data_ctg$workers.geih, frequency = 12)
...
arima_model <- auto.arima(workers_ts, seasonal = FALSE, d=0) #Letting auto.arima find the best predictors
...
```
*ACF*: The ACF plot shows the autocorrelation of the series with its lags. Ideally, for a time series that has no autocorrelation, we would expect to see the plot drop to near-zero quickly. In this case, after differencing three times, the ACF plot shows that the autocorrelations quickly fall within the confidence bounds, which indicates no significant autocorrelation in the differenced series.

*PACF*: Here, we see that most PACF values are within the confidence bounds, suggesting that all the partial autocorrelations within the confidence bounds, suggesting no additional AR terms are needed for the model.
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_ctg$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_ctg$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_ctg$workers.geih))

# Define the time series plot range
time_range <- range(cartagena_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(cartagena_dataset_subset$workers.geih, quantile(test_data_ctg$predicted_workers,0.25), test_data_ctg$predicted_workers_sig, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(cartagena_dataset_subset$date, cartagena_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in CARTAGENA")
...
mae_arima <- mean(abs(test_data_ctg$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_ctg$workers.geih - forecasts$mean)^2))
...
summary(forecasts)
```
We can visually confirm that one of the ARIMA model is predicting the variables better than one of the prior 2 models but the ARIMA models with the d argument manually set to 3 is rising exponentially and the one determined by the auto.arima function is just producing a straight line. Let's proceed to store the evaluation results of each to be able to better compare them at the end.
```{r}
...
ols_mae <- mean(abs(test_data_ctg$predicted_workers - test_data_ctg$workers.geih))
ols_rmse <- sqrt(mean((test_data_ctg$predicted_workers - test_data_ctg$workers.geih)^2))
ols_summary <- summary(stepwise_model_ctg)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 1.956e-07, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_ctg),
  AICc = AIC(stepwise_model_ctg) + (2 * length(coef(stepwise_model_ctg)) * (length(coef(stepwise_model_ctg)) + 1)) / (length(resid(stepwise_model_ctg)) - length(coef(stepwise_model_ctg)) - 1),
  BIC = BIC(stepwise_model_ctg)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_ctg$predicted_workers_sig - test_data_ctg$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_ctg$predicted_workers_sig - test_data_ctg$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_ctg_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 1.904e-09, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_ctg_sig),
  AICc = AIC(stepwise_model_ctg_sig) + (2 * length(coef(stepwise_model_ctg_sig)) * (length(coef(stepwise_model_ctg_sig)) + 1)) / (length(resid(stepwise_model_ctg_sig)) - length(coef(stepwise_model_ctg_sig)) - 1),
  BIC = BIC(stepwise_model_ctg_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_ctg$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_ctg$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_ctg$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_ctg$workers.geih - forecasts_d$mean)^2))
...
```
As we can see, the auto Arima model has a better performance than the other models, but it might not be still the most optimal one. Let's see how the residuals of our auto ARIMA prediction, as well as our ACF and PACF plots look like in order to determine our next steps.

```{r Residual and ACF / PACF Analysis of ARIMA}
# Calculate residuals
residuals <- test_data_ctg$workers.geih - forecasts$mean
...
*ACF*: Used to show the correlation of the time series with its own lagged values.  It shows that most of the autocorrelations for various lags are within the blue confidence limits, which suggests that there is little autocorrelation remaining in the residuals. 
*PACF*: Used for identifying the number of AR (Autoregressive) terms. Most of the spikes fall within the blue dotted confidence bounds, which suggests that there may be no significant partial autocorrelations at those lags.
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_ctg$workers.geih))
```
...
workers_range <- range(cartagena_dataset_subset$workers.geih, 
                       test_data_ctg$predicted_workers, 
                       test_data_ctg$predicted_workers_sig, 
                       all_confidence_intervals)

plot(cartagena_dataset_subset$date, cartagena_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in CARTAGENA")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_ctg$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_ctg$date, test_data_ctg$predicted_workers, col='red', lwd=1.8)
lines(test_data_ctg$date, test_data_ctg$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(test_data_ctg$date)), col="black", lwd=1.8, lty=2)
...
sarima_mae <- mean(abs(test_data_ctg$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_ctg$workers.geih - sarima_forecasts$mean)^2))
...
print(model_summaries)
```
The SARIMA model gave us a better result than the ARIMA predictions. Let's see if we can obtain better results fitting a Random Forrest Model to see if it can outperform the models we have so far.
```{r Random forrest}
# Load data
data_rf <- cartagena_dataset_subset
...
# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cartagena_dataset_subset$workers.geih, 480000))

# Plot the actual data for the complete dataset
plot(cartagena_dataset_subset$date, cartagena_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=full_range,  main="Forecasts Comparison for Employed Workers in CARTAGENA")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# ARIMA d=3 forecast line
lines(arima_forecast_dates, forecasts_d$mean, col='darkviolet', lwd=1.8)
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_ctg)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_ctg_sig)
...
write_xlsx(model_summaries, path = "cartagena_results.xlsx")
...
png("cartagena_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
