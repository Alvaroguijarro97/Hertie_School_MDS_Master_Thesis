...
Let's proceed to work on the prediction for the city of PASTO
```{r}
pasto_dataset <- employed_data %>%
  filter( employed_data$city == "PASTO") %>%
...
```{r}
pasto_dataset$date <- as.Date(paste0(pasto_dataset$date, "-01"), format = "%Y-%m-%d")
plot(pasto_dataset$date, pasto_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
pas_basic_ols <- lm(workers.geih ~ . - date, data = pasto_dataset)
summary(pas_basic_ols)
...
pasto_dataset_subset <- pasto_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(pasto_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(pasto_dataset_subset$workers.geih)
qqline(pasto_dataset_subset$workers.geih, col = "red")
shapiro.test(pasto_dataset_subset$workers.geih)
summary(pasto_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = pasto_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(pasto_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
pasto_dataset_subset$date <- as.Date(paste0(pasto_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_pas <- pasto_dataset_subset[pasto_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_pas <- pasto_dataset_subset[pasto_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_pas <- lm(workers.geih ~ . - date, data = train_data_pas)

# Summarize the model to look at coefficients
summary(model_pas)

```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 0.9861 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_pas <- lm(workers.geih ~ . - date, data = train_data_pas)
stepwise_model_pas <- step(model_pas, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_pas)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_pas$predicted_workers <- predict(stepwise_model_pas, newdata = test_data_pas)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(pasto_dataset_subset$workers.geih, mean(test_data_pas$predicted_workers)))

# Plot the actual data with the adjusted y limits
plot(pasto_dataset_subset$date, pasto_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(pasto_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_pas$date, test_data_pas$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_pas))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_pas), resid(stepwise_model_pas), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_pas)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_pas)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_pas$predicted_workers - test_data_pas$workers.geih))
rmse <- sqrt(mean((test_data_pas$predicted_workers - test_data_pas$workers.geih)^2))
...
W = 0.97848, p-value = 0.769: The p-value is greater than 0.05, so we do not reject the null hypothesis of normality. This indicates that the residuals of the model may be considered normally distributed, which satisfies one of the OLS assumptions.
...
DW = 2.9591, p-value = 0.7498: A Durbin-Watson statistic close to 3 suggests that there is a negative autocorrelation in the residuals, which doesn't meets the assumption of independence. The p-value here is above 0.05, indicating some evidence of autocorrelation. This could be problematic for the validity of the OLS estimates.
...
MAE: 187593.7 and RMSE: 298531.9  suggest that the model's predictions deviate from the actual values by a large margin. Lower values of MAE and RMSE would indicate better predictive accuracy. The large values here indicate the model may not predict the target variable very well.
...
```{r}
coef_summary <- summary(stepwise_model_pas)$coefficients
...
stepwise_model_pas_sig <- lm(significant_formula, data = train_data_pas)

# Summary of the new model
summary(stepwise_model_pas_sig)

```

```{r}
# Predicting with the stepwise model
test_data_pas$predicted_workers_sig <- predict(stepwise_model_pas_sig, newdata = test_data_pas)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(pasto_dataset_subset$workers.geih, mean(test_data_pas$predicted_workers)))

# Plot the actual data for the complete dataset
plot(pasto_dataset_subset$date, pasto_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_pas$date, test_data_pas$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_pas$date, test_data_pas$predicted_workers_sig, col='gold')
...
xlim <- range(pasto_dataset_subset$date)
ylim <- range(c(pasto_dataset_subset$workers.geih, test_data_pas$predicted_workers, test_data_pas$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_pas_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_pas_sig), resid(stepwise_model_pas_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_pas_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_pas_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_pas$predicted_workers_sig - test_data_pas$workers.geih))
rmse_sig <- sqrt(mean((test_data_pas$predicted_workers_sig - test_data_pas$workers.geih)^2))
...
workers_ts <- ts(train_data_pas$workers.geih, frequency = 12)
...
```
After differencing we can see that our p-value under threshold of 0.05, we can assume that it is stationary and proceed to predict our variable.  
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_pas$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_pas$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_pas$workers.geih))

# Define the time series plot range
time_range <- range(pasto_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(pasto_dataset_subset$workers.geih,180000)

# Plot the actual data for the complete dataset
plot(pasto_dataset_subset$date, pasto_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in PASTO")
...
mae_arima <- mean(abs(test_data_pas$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_pas$workers.geih - forecasts$mean)^2))
...
ols_mae <- mean(abs(test_data_pas$predicted_workers - test_data_pas$workers.geih))
ols_rmse <- sqrt(mean((test_data_pas$predicted_workers - test_data_pas$workers.geih)^2))
ols_summary <- summary(stepwise_model_pas)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 6.913e-13, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_pas),
  AICc = AIC(stepwise_model_pas) + (2 * length(coef(stepwise_model_pas)) * (length(coef(stepwise_model_pas)) + 1)) / (length(resid(stepwise_model_pas)) - length(coef(stepwise_model_pas)) - 1),
  BIC = BIC(stepwise_model_pas)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_pas$predicted_workers_sig - test_data_pas$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_pas$predicted_workers_sig - test_data_pas$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_pas_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 3.445e-13, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_pas_sig),
  AICc = AIC(stepwise_model_pas_sig) + (2 * length(coef(stepwise_model_pas_sig)) * (length(coef(stepwise_model_pas_sig)) + 1)) / (length(resid(stepwise_model_pas_sig)) - length(coef(stepwise_model_pas_sig)) - 1),
  BIC = BIC(stepwise_model_pas_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_pas$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_pas$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_pas$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_pas$workers.geih - forecasts_d$mean)^2))
...
# Calculate residuals
residuals <- test_data_pas$workers.geih - forecasts$mean
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_pas$workers.geih))
```
...
workers_range <- range(pasto_dataset_subset$workers.geih,160000)

plot(pasto_dataset_subset$date, pasto_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in PASTO")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_pas$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_pas$date, test_data_pas$predicted_workers, col='red', lwd=1.8)
lines(test_data_pas$date, test_data_pas$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(test_data_pas$date)), col="black", lwd=1.8, lty=2)
...
sarima_mae <- mean(abs(test_data_pas$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_pas$workers.geih - sarima_forecasts$mean)^2))
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_pas)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_pas_sig)
...
write_xlsx(model_summaries, path = "pasto_results.xlsx")
...
png("pasto_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
