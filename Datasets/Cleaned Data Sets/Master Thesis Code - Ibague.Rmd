...
Let's proceed to work on the prediction for the city of IBAGUÉ
```{r}
ibague_dataset <- employed_data %>%
  filter( employed_data$city == "IBAGUÉ") %>%
...
```{r}
ibague_dataset$date <- as.Date(paste0(ibague_dataset$date, "-01"), format = "%Y-%m-%d")
plot(ibague_dataset$date, ibague_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
iba_basic_ols <- lm(workers.geih ~ . - date, data = ibague_dataset)
summary(iba_basic_ols)
...
ibague_dataset_subset <- ibague_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(ibague_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(ibague_dataset_subset$workers.geih)
qqline(ibague_dataset_subset$workers.geih, col = "red")
shapiro.test(ibague_dataset_subset$workers.geih)
summary(ibague_dataset_subset$workers.geih)
```
...
```{r}
variables <- c("year", "month", "population_month.pop", "CPI.cpi", "CPI_month_var.cpi", "Enrollment_Rate_5_16.edu", "Net_Coverage.edu", "Pass_Rate.edu", "I_PM.mp", "I_PME.mp", "Gini.mp", "IPUG.mp", "LP.mp", "LPE.mp", "MDM_Resource_Mobilization.ci", "MDM_Execution_Of_Resources.ci", "MDM_Open_Government_And_Transparency.ci", "MDM_Territorial_Ordering.ci", "MDM_Education.ci", "MDM_Health_Coverage.ci", "MDM_Services.ci", "MDM_Security_And_Coexistence.ci", "TotalIncome.fp", "TotalExpenses.fp", "Self_financing_of_operating_expenses.sfp", "Debt_service_support.sfp", "Dependence_on_transfers_from_the_Nation_and_Royalties.sfp", "Generation_of_Own_Resources.sfp", "Magnitude_of_Investment.sfp", "Saving_Capacity.sfp", "Fiscal_Performance_Indicator.sfp")

for (variable in variables) {
  # Fit a linear model for each variable
  formula <- as.formula(paste("workers.geih ~", variable))
  model <- lm(formula, data = ibague_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(ibague_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
ibague_dataset_subset$date <- as.Date(paste0(ibague_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_iba <- ibague_dataset_subset[ibague_dataset_subset$date < as.Date("2019-07-01"), ]
train_data_iba <- ibague_dataset_subset[ibague_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_iba <- lm(workers.geih ~ . - date, data = train_data_iba)

# Summarize the model to look at coefficients
summary(model_iba)
```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 1 suggest  over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

We need to reduce the number of variables we have on this model.

```{r}
selected_variables_2 <- c("workers.geih","date", "month", "population_month.pop", "MDM_Resource_Mobilization.ci", "MDM_Execution_Of_Resources.ci")

# Subset the dataframe using the vector of selected variables
ibague_dataset_subset_2 <- ibague_dataset[, selected_variables_2]
```

```{r}
#First, let's split the data into training and testing
ibague_dataset_subset_2$date <- as.Date(paste0(ibague_dataset_subset_2$date, "-01"), format = "%Y-%m-%d")
train_data_iba_2 <- ibague_dataset_subset_2[ibague_dataset_subset$date < as.Date("2019-07-01"), ]
train_data_iba_2 <- ibague_dataset_subset_2[ibague_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_iba_2 <- lm(workers.geih ~ . - date, data = train_data_iba_2)
...
```{r}
model_iba_2 <- lm(workers.geih ~ . - date, data = train_data_iba_2)
stepwise_model_iba <- step(model_iba_2, direction = "forward", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_iba)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
train_data_iba$predicted_workers <- predict(stepwise_model_iba, newdata = train_data_iba)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(ibague_dataset_subset$workers.geih, mean(train_data_iba$predicted_workers)))

# Plot the actual data with the adjusted y limits
plot(ibague_dataset_subset$date, ibague_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(ibague_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(train_data_iba$date, train_data_iba$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_iba))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_iba), resid(stepwise_model_iba), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_iba)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_iba)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(train_data_iba$predicted_workers - train_data_iba$workers.geih))
rmse <- sqrt(mean((train_data_iba$predicted_workers - train_data_iba$workers.geih)^2))
...
W = 0.97292, p-value = 0.9114: The p-value is greater than 0.05, so we do not reject the null hypothesis of normality. This indicates that the residuals of the model may be considered normally distributed, which satisfies one of the OLS assumptions.
...
MAE: 513.3566  and RMSE: 612.3239  suggest that the model's predictions deviate from the actual values by a relative small margin. Lower values of MAE and RMSE would indicate better predictive accuracy. 
...
```{r}
coef_summary <- summary(stepwise_model_iba)$coefficients

#see what happens if we take away the month variable
#significant_vars <- selected_variables_2 %>% select("date")
...
stepwise_model_iba_sig <- lm(workers.geih ~ date + population_month.pop + MDM_Resource_Mobilization.ci + MDM_Execution_Of_Resources.ci, data = train_data_iba)

# Summary of the new model
summary(stepwise_model_iba_sig)

```
```{r}
# Predicting with the stepwise model
train_data_iba$predicted_workers_sig <- predict(stepwise_model_iba_sig, newdata = train_data_iba)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(ibague_dataset_subset$workers.geih, mean(train_data_iba$predicted_workers)))

# Plot the actual data for the complete dataset
plot(ibague_dataset_subset$date, ibague_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(train_data_iba$date, train_data_iba$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(train_data_iba$date, train_data_iba$predicted_workers_sig, col='gold')
...
xlim <- range(ibague_dataset_subset$date)
ylim <- range(c(ibague_dataset_subset$workers.geih, train_data_iba$predicted_workers, train_data_iba$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_iba_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_iba_sig), resid(stepwise_model_iba_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_iba_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_iba_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(train_data_iba$predicted_workers_sig - train_data_iba$workers.geih))
rmse_sig <- sqrt(mean((train_data_iba$predicted_workers_sig - train_data_iba$workers.geih)^2))
...
```
Even though the MAE and RMSE Result show an improvement, we can visually see that the model is effectively predicting our workers variable. We are going to analyze another more robust prediction models that might capture better the behavior of our variables and could produce more trustworthy predictions. In this case we are going to analyze ARIMA, SARIMA, and Random Forrest models. 
...
workers_ts <- ts(train_data_iba$workers.geih, frequency = 12)
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(train_data_iba$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(train_data_iba$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(train_data_iba$workers.geih))

# Define the time series plot range
time_range <- range(ibague_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(ibague_dataset_subset$workers.geih, quantile(train_data_iba$predicted_workers,0.25), train_data_iba$predicted_workers_sig, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(ibague_dataset_subset$date, ibague_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in IBAGUE A.M.")
...
mae_arima <- mean(abs(train_data_iba$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((train_data_iba$workers.geih - forecasts$mean)^2))
...
ols_mae <- mean(abs(train_data_iba$predicted_workers - train_data_iba$workers.geih))
ols_rmse <- sqrt(mean((train_data_iba$predicted_workers - train_data_iba$workers.geih)^2))
ols_summary <- summary(stepwise_model_iba)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 0.4625, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_iba),
  AICc = AIC(stepwise_model_iba) + (2 * length(coef(stepwise_model_iba)) * (length(coef(stepwise_model_iba)) + 1)) / (length(resid(stepwise_model_iba)) - length(coef(stepwise_model_iba)) - 1),
  BIC = BIC(stepwise_model_iba)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(train_data_iba$predicted_workers_sig - train_data_iba$workers.geih))
sig_ols_rmse <- sqrt(mean((train_data_iba$predicted_workers_sig - train_data_iba$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_iba_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 0.2914, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_iba_sig),
  AICc = AIC(stepwise_model_iba_sig) + (2 * length(coef(stepwise_model_iba_sig)) * (length(coef(stepwise_model_iba_sig)) + 1)) / (length(resid(stepwise_model_iba_sig)) - length(coef(stepwise_model_iba_sig)) - 1),
  BIC = BIC(stepwise_model_iba_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(train_data_iba$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((train_data_iba$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(train_data_iba$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((train_data_iba$workers.geih - forecasts_d$mean)^2))
...
# Calculate residuals
residuals <- train_data_iba$workers.geih - forecasts$mean
...
sarima_forecasts <- forecast(sarima_model, h = length(train_data_iba$workers.geih))
```
...
workers_range <- range(ibague_dataset_subset$workers.geih, 
                       quantile(train_data_iba$predicted_workers,0.25), 
                       train_data_iba$predicted_workers_sig, 
                       all_confidence_intervals)

plot(ibague_dataset_subset$date, ibague_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in IBAGUE A.M.")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(train_data_iba$date), 
                             by = "month", 
...
# OLS forecasted data
lines(train_data_iba$date, train_data_iba$predicted_workers, col='red', lwd=1.8)
lines(train_data_iba$date, train_data_iba$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(train_data_iba$date)), col="black", lwd=1.8, lty=2)
...
sarima_mae <- mean(abs(train_data_iba$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((train_data_iba$workers.geih - sarima_forecasts$mean)^2))
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_iba)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_iba_sig)
...
write_xlsx(model_summaries, path = "ibague_results.xlsx")
...
png("ibague_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
