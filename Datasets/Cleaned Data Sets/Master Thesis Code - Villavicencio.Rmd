...
Let's proceed to work on the prediction for the city of VILLAVICENCIO
```{r}
villavicencio_dataset <- employed_data %>%
  filter( employed_data$city == "VILLAVICENCIO") %>%
  select(-city, -eco_activity, -Category.sfp, -lat.ll, -lat_z.ll, -lng.ll, -lng_z.ll)
villavicencio_dataset
```
Let's see again how does our variable of interest looks like: 
```{r}
villavicencio_dataset$date <- as.Date(paste0(villavicencio_dataset$date, "-01"), format = "%Y-%m-%d")
plot(villavicencio_dataset$date, villavicencio_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
baq_basic_ols <- lm(workers.geih ~ . - date, data = villavicencio_dataset)
summary(baq_basic_ols)
...
villavicencio_dataset_subset <- villavicencio_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(villavicencio_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(villavicencio_dataset_subset$workers.geih)
qqline(villavicencio_dataset_subset$workers.geih, col = "red")
shapiro.test(villavicencio_dataset_subset$workers.geih)
summary(villavicencio_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = villavicencio_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(villavicencio_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
villavicencio_dataset_subset$date <- as.Date(paste0(villavicencio_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_vil <- villavicencio_dataset_subset[villavicencio_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_vil <- villavicencio_dataset_subset[villavicencio_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_vil <- lm(workers.geih ~ . - date, data = train_data_vil)

# Summarize the model to look at coefficients
summary(model_vil)

```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 0.9653 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_vil <- lm(workers.geih ~ . - date, data = train_data_vil)
stepwise_model_vil <- step(model_vil, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_vil)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_vil$predicted_workers <- predict(stepwise_model_vil, newdata = test_data_vil)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(villavicencio_dataset_subset$workers.geih, mean(test_data_vil$predicted_workers)))

# Plot the actual data with the adjusted y limits
plot(villavicencio_dataset_subset$date, villavicencio_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(villavicencio_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_vil$date, test_data_vil$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_vil))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_vil), resid(stepwise_model_vil), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_vil)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_vil)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_vil$predicted_workers - test_data_vil$workers.geih))
rmse <- sqrt(mean((test_data_vil$predicted_workers - test_data_vil$workers.geih)^2))
...
W = 0.9858, p-value = 0.945: The p-value is greater than 0.05, so we do not reject the null hypothesis of normality. This indicates that the residuals of the model may be considered normally distributed, which satisfies one of the OLS assumptions.
*Homoscedasticity (Plot of Residuals vs. Fitted Values)*
The residuals plot shows a random dispersion of points around the horizontal line at zero, which suggests that there might not be any obvious problems with homoscedasticity. Nevertheless, it is important to note the potential clustering of data points around the center. 

*Independence (Durbin-Watson test)*
DW = 1.9917, p-value = 0.0427: A Durbin-Watson statistic close to 2 suggests that there is no autocorrelation in the residuals, which meets the assumption of independence. However, the p-value here is below 0.05, indicating some evidence of positive autocorrelation. This could be problematic for the validity of the OLS estimates.
*Multicollinearity (VIF)*
The reported VIF values are high, indicating potential multicollinearity among the predictors. Particularly, variables with VIF scores in the thousand or millions far exceed common thresholds of 5 or 10, suggesting that multicollinearity might be inflating the variances of the parameter estimates and could be problematic for the model.
*Prediction Performance (MAE and RMSE)*
MAE: 12414.46 and RMSE: 17146.6 suggest that the model's predictions deviate from the actual values by a large margin. Lower values of MAE and RMSE would indicate better predictive accuracy. The large values here indicate the model may not predict the target variable very well.
...
```{r}
coef_summary <- summary(stepwise_model_vil)$coefficients

# Extract names of statistically significant variables (p-value < 0.05)
significant_vars <- rownames(coef_summary)[coef_summary[, "Pr(>|t|)"] < 0.057]
...
stepwise_model_vil_sig <- lm(significant_formula, data = train_data_vil)

# Summary of the new model
summary(stepwise_model_vil_sig)

```
```{r}
# Predicting with the stepwise model
test_data_vil$predicted_workers_sig <- predict(stepwise_model_vil_sig, newdata = test_data_vil)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(villavicencio_dataset_subset$workers.geih, mean(test_data_vil$predicted_workers)))

# Plot the actual data for the complete dataset
plot(villavicencio_dataset_subset$date, villavicencio_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_vil$date, test_data_vil$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_vil$date, test_data_vil$predicted_workers_sig, col='gold')
...
xlim <- range(villavicencio_dataset_subset$date)
ylim <- range(c(villavicencio_dataset_subset$workers.geih, test_data_vil$predicted_workers, test_data_vil$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_vil_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_vil_sig), resid(stepwise_model_vil_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_vil_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_vil_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_vil$predicted_workers_sig - test_data_vil$workers.geih))
rmse_sig <- sqrt(mean((test_data_vil$predicted_workers_sig - test_data_vil$workers.geih)^2))
...
```
Even though the MAE and RMSE Result show an improvement, we can visually see that the model is not effectively predicting our workers variable because it is not picking up the seasonal component of our data. We are going to analyze another more robust prediction models that might capture better the behavior of our variables and could produce more trustworthy predictions. In this case we are going to analyze ARIMA, SARIMA, and Random Forrest models. 
...
```
The first round of differencing gave us a p-value above 0.05, so we need to apply differencing again in order to make it stationary.

```{r}
# Apply further differencing if necessary
diff_workers_ts_2 <- diff(diff_workers_ts)
adf.test(diff_workers_ts_2, alternative = "stationary")
```
```{r}
# Apply further differencing if necessary
diff_workers_ts_3 <- diff(diff_workers_ts_2)
adf.test(diff_workers_ts_3, alternative = "stationary")
```
...
acf(diff_workers_ts_3)
pacf(diff_workers_ts_3)
...
arima_model_d<- auto.arima(workers_ts, seasonal = FALSE, d=3) #setting the order of first-differencing to 3

summary(arima_model)
print("")
summary(arima_model_d)
```
*ACF*: The ACF plot shows the autocorrelation of the series with its lags. Ideally, for a time series that has no autocorrelation, we would expect to see the plot drop to near-zero quickly. In this case, after differencing three time, the ACF plot shows that most of the autocorrelations are within the confidence interval (the blue dotted lines), which is an indication that there may not be significant autocorrelation in the differenced series.
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_vil$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_vil$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_vil$workers.geih))

# Define the time series plot range
time_range <- range(villavicencio_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(villavicencio_dataset_subset$workers.geih, quantile(test_data_vil$predicted_workers,0.25), test_data_vil$predicted_workers_sig, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(villavicencio_dataset_subset$date, villavicencio_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in VILLAVICENCIO")
...
legend("topleft", inset=c(0, 0), # Negative inset x moves legend to the right
       legend=c("Actual (2016-2019)", "OLS Prediction (2019-07 onward)", 
                "OLS Prediction (2019-07 onward) Sig. Variables", "ARIMA Forecast", "ARIMA Forecast d=3",
...
mae_arima <- mean(abs(test_data_vil$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_vil$workers.geih - forecasts$mean)^2))
...
ols_mae <- mean(abs(test_data_vil$predicted_workers - test_data_vil$workers.geih))
ols_rmse <- sqrt(mean((test_data_vil$predicted_workers - test_data_vil$workers.geih)^2))
ols_summary <- summary(stepwise_model_vil)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 2.008e-10, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_vil),
  AICc = AIC(stepwise_model_vil) + (2 * length(coef(stepwise_model_vil)) * (length(coef(stepwise_model_vil)) + 1)) / (length(resid(stepwise_model_vil)) - length(coef(stepwise_model_vil)) - 1),
  BIC = BIC(stepwise_model_vil)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_vil$predicted_workers_sig - test_data_vil$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_vil$predicted_workers_sig - test_data_vil$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_vil_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 0.0008177, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_vil_sig),
  AICc = AIC(stepwise_model_vil_sig) + (2 * length(coef(stepwise_model_vil_sig)) * (length(coef(stepwise_model_vil_sig)) + 1)) / (length(resid(stepwise_model_vil_sig)) - length(coef(stepwise_model_vil_sig)) - 1),
  BIC = BIC(stepwise_model_vil_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_vil$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_vil$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_vil$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_vil$workers.geih - forecasts_d$mean)^2))
arima_d_summary <- summary(arima_model_d)

# Add ARIMA model metrics to the data frame
model_summaries <- rbind(model_summaries, data.frame(
  Model = "ARIMA with d=3",
  MAE = arima_d_mae,
  RMSE = arima_d_rmse,
  R_Squared = NA, # ARIMA models do not have R-squared
  P_Value = NA,   # P-value usually not directly applicable to ARIMA models
...
```
As we can see, the ARIMA model have not improved the prediction performance than the other models, and they are still not the most optimal ones. Let's see how the residuals of our auto ARIMA prediction, as well as our ACF and PACF plots look like in order to determine our next steps.

```{r Residual and ACF / PACF Analysis of ARIMA}
# Calculate residuals
residuals <- test_data_vil$workers.geih - forecasts$mean
...
*Residuals*: Use to visually assess the randomness and distribution of the residuals in our prediction. The plot of the residuals of the ARIMA model appears to exhibit some pattern, as indicated by the significant swings above and below the zero line. This suggests that the model may not be fully capturing the underlying data structure, which might be due to seasonality or another form of autocorrelation not addressed by the model.
*ACF*: Used to show the correlation of the time series with its own lagged values.  It shows that most of the autocorrelations for various lags are within the blue confidence limits, which suggests that there is little autocorrelation remaining in the residuals. However, there are a couple of lags that exceed the confidence bounds, which indicates that some autocorrelation may still be present.
*PACF*: Used for identifying the number of AR (Autoregressive) terms. There are a few lags with significant partial autocorrelations, suggesting that the ARIMA model could potentially benefit from including additional AR terms to better capture the data structure.
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_vil$workers.geih))
```
...
workers_range <- range(villavicencio_dataset_subset$workers.geih, 
                       quantile(test_data_vil$predicted_workers,0.25), 
                       test_data_vil$predicted_workers_sig, 
                       all_confidence_intervals)

plot(villavicencio_dataset_subset$date, villavicencio_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in VILLAVICENCIO")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_vil$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_vil$date, test_data_vil$predicted_workers, col='red', lwd=1.8)
lines(test_data_vil$date, test_data_vil$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(test_data_vil$date)), col="black", lwd=1.8, lty=2)

# Update the legend with SARIMA
legend("topleft", inset=c(0, 0), # Negative inset x moves legend to the right
       legend=c("Actual (2016-2019)", "OLS Prediction (2019-07 onward)", 
                "OLS Prediction (2019-07 onward) Sig. Variables", "ARIMA Forecast","ARIMA d=3 Forecast", 
                "SARIMA Forecast"), 
...
sarima_mae <- mean(abs(test_data_vil$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_vil$workers.geih - sarima_forecasts$mean)^2))
...
print(model_summaries)
```
The SARIMA model gave us better results than the initial ARIMA prediction. Let's see if we can obtain better results fitting a Random Forrest Model to see if it can outperform the models we have so far.
```{r Random forrest}
# Load data
data_rf <- villavicencio_dataset_subset
...
rf_model_rmse <- train(
  ...
  tuneLength = 10,
  metric = "RMSE",
  importance = 'impurity' # This tells ranger to calculate variable importance
)
...
lines(test_dates, predictions_rmse, col='darkgreen', lwd=1.8, lty=2)
...
legend("topleft", inset=c(0, 0), # Negative inset x moves legend to the right
       legend=c("Actual (2016-2019)", "OLS Prediction (2019-07 onward)", 
                "OLS Prediction (2019-07 onward) Sig. Variables", "ARIMA Forecast","ARIMA d=3 Forecast", 
                "SARIMA Forecast", "RF Prediction","RF Prediction (Best RMSE)", "RF Prediction (Best R-squared)"), 
       col=c("black", "red", "gold", "blue", "darkviolet", "green", "orange", 'darkgreen',"cyan"), 
       lty=c(1, 1, 1, 1, 1, 1, 1, 2, 2), 
       lwd=c(2, 2, 2, 2, 2, 2, 2, 2, 2), 
       merge=TRUE, 
       cex=0.7, 
       fill=c("black", "red", "gold", "blue", "darkviolet", "green", "orange", 'darkgreen', "cyan"),
       bty="n") 
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_vil)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_vil_sig)
...
```
```{r importance for RF Model}
# Convert the matrix to a data frame
importance_df <- as.data.frame(importance)

# Add the feature names as a new column from the row names
importance_df$Feature <- rownames(importance_df)

# Sort the data frame by importance
importance_df <- importance_df[order(-importance_df$IncNodePurity), ]

# Select the top N features for clarity
top_n <- 10
importance_top_n <- head(importance_df, top_n)

# Increase bottom margin to accommodate angled names
par(mar=c(7, 4, 4, 2) + 0.1)

# Plot without names
bp <- barplot(importance_top_n$IncNodePurity, 
        main="Feature Importance in Random Forest Model",
        las=2) 

# Add names at a 45-degree angle
text(bp, par("usr")[3] - 0.5, srt = 45, adj = 1,
     labels = importance_top_n$Feature, xpd = TRUE, cex = 0.75)

# Reset the default par settings after plotting
par(mfrow=c(1,1), mar=c(5.1, 4.1, 4.1, 2.1))
```
```{r Get RF Models}
# Extract the ranger model from the caret train object
ranger_model_rf <- rf_model$finalModel
ranger_model_rmse <- rf_model_rmse$finalModel
ranger_model_rsquared <- rf_model_rsquared$finalModel

# Now you can calculate variable importance directly from the ranger model
importance_rf <- ranger_model_rf$variable.importance
importance_rmse <- ranger_model_rmse$variable.importance
importance_rsquared <- ranger_model_rsquared$variable.importance

# The importance is returned as a named vector, you can sort it to see the most important variables
sorted_importance_rf <- sort(importance_rf, decreasing = TRUE)
sorted_importance_rmse <- sort(importance_rmse, decreasing = TRUE)
sorted_importance_rsquared <- sort(importance_rsquared, decreasing = TRUE)

# To view the importance
print(sorted_importance_rf)
print("")
print(sorted_importance_rmse)
print("")
print(sorted_importance_rsquared)

```

```{r rf RMSE x}
# Convert the matrix to a data frame
importance_r2_df <- as.data.frame(sorted_importance_rmse)

# Add the feature names as a new column from the row names
importance_r2_df$Feature <- rownames(importance_r2_df)

# Select the top N features for clarity
top_n <- 10
importance_r2_top_n <- head(importance_r2_df, top_n)

# Increase bottom margin to accommodate angled names
par(mar=c(7, 4, 4, 2) + 0.1)

# Plot without names
bp <- barplot(importance_r2_top_n$sorted_importance_rmse, 
        main="Feature Importance in Random Forest Model focused on RMSE",
        las=2) 

# Add names at a 45-degree angle
text(bp, par("usr")[3] - 0.5, srt = 45, adj = 1,
     labels = importance_r2_top_n$Feature, xpd = TRUE, cex = 0.75)
...
```{r rf Rsquared x}
# Convert the matrix to a data frame
importance_r3_df <- as.data.frame(sorted_importance_rsquared)

# Add the feature names as a new column from the row names
importance_r3_df$Feature <- rownames(importance_r3_df)

# Select the top N features for clarity
top_n <- 10
importance_r3_top_n <- head(importance_r3_df, top_n)

# Increase bottom margin to accommodate angled names
par(mar=c(7, 4, 4, 2) + 0.1)

# Plot without names
bp <- barplot(importance_r3_top_n$sorted_importance_rsquared, 
        main="Feature Importance in Random Forest Model focused on R squared",
        las=2) 

# Add names at a 45-degree angle
text(bp, par("usr")[3] - 0.5, srt = 45, adj = 1,
     labels = importance_r3_top_n$Feature, xpd = TRUE, cex = 0.5)

# Reset the default par settings after plotting
par(mfrow=c(1,1), mar=c(5.1, 4.1, 4.1, 2.1))
```

```{r export result summaries}
# Assuming 'df' is your dataframe
write_xlsx(model_summaries, path = "villavicencio_results.xlsx")
...
png("villavicencio_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
