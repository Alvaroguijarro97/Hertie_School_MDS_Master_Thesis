...
Let's proceed to work on the prediction for the city of MANIZALES A.M.
```{r}
manizales_dataset <- employed_data %>%
  filter( employed_data$city == "MANIZALES A.M.") %>%
...
```{r}
manizales_dataset$date <- as.Date(paste0(manizales_dataset$date, "-01"), format = "%Y-%m-%d")
plot(manizales_dataset$date, manizales_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
man_basic_ols <- lm(workers.geih ~ . - date, data = manizales_dataset)
...
manizales_dataset_subset <- manizales_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(manizales_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(manizales_dataset_subset$workers.geih)
qqline(manizales_dataset_subset$workers.geih, col = "red")
shapiro.test(manizales_dataset_subset$workers.geih)
summary(manizales_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = manizales_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(manizales_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
manizales_dataset_subset$date <- as.Date(paste0(manizales_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_man <- manizales_dataset_subset[manizales_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_man <- manizales_dataset_subset[manizales_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_man <- lm(workers.geih ~ . - date, data = train_data_man)

# Summarize the model to look at coefficients
summary(model_man)

```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 0.9571 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_man <- lm(workers.geih ~ . - date, data = train_data_man)
stepwise_model_man <- step(model_man, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_man)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_man$predicted_workers <- predict(stepwise_model_man, newdata = test_data_man)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(manizales_dataset_subset$workers.geih, mean(test_data_man$predicted_workers)))

# Plot the actual data with the adjusted y limits
plot(manizales_dataset_subset$date, manizales_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(manizales_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_man$date, test_data_man$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_man))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_man), resid(stepwise_model_man), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_man)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_man)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_man$predicted_workers - test_data_man$workers.geih))
rmse <- sqrt(mean((test_data_man$predicted_workers - test_data_man$workers.geih)^2))
...
W = 0.98907, p-value = 0.9838: The very high p-value suggests that we fail to reject the null hypothesis that the residuals are normally distributed. This indicates that the residuals of your model are consistent with a normal distribution, fulfilling the normality assumption required for OLS regression.
*Homoscedasticity (Plot of Residuals vs. Fitted Values)*
The residuals appear to be relatively evenly scattered above and below the horizontal line at zero, which is a good sign that the assumption may not be violated.It appears that residuals scatter evenly across the range of fitted values without any discernible pattern or funnel shape that would indicate heteroscedasticity. 
*Independence (Durbin-Watson test)*
DW = 1.7641, p-value = 0.0008615: The Durbin-Watson statistic is close to 2, which typically indicates no autocorrelation, but the very low p-value suggests that there is statistically significant evidence of positive autocorrelation in the residuals. This could be a concern as it violates the OLS assumption of independent errors.
...
MAE: 72773.14 and RMSE: 103806: These metrics suggest that on average, the model’s predictions are off by a substantial margin from the actual values. Ideally, we would want both MAE and RMSE to be as low as possible. The high values here indicate that the model's predictive accuracy is limited
...
```{r}
coef_summary <- summary(stepwise_model_man)$coefficients
...
stepwise_model_man_sig <- lm(significant_formula, data = train_data_man)

# Summary of the new model
summary(stepwise_model_man_sig)
```

```{r}
# Predicting with the stepwise model
test_data_man$predicted_workers_sig <- predict(stepwise_model_man_sig, newdata = test_data_man)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(250000,manizales_dataset_subset$workers.geih))

# Plot the actual data for the complete dataset
plot(manizales_dataset_subset$date, manizales_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_man$date, test_data_man$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_man$date, test_data_man$predicted_workers_sig, col='gold')
...
xlim <- range(manizales_dataset_subset$date)
ylim <- range(c(manizales_dataset_subset$workers.geih, test_data_man$predicted_workers, test_data_man$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_man_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_man_sig), resid(stepwise_model_man_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_man_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_man_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_man$predicted_workers_sig - test_data_man$workers.geih))
rmse_sig <- sqrt(mean((test_data_man$predicted_workers_sig - test_data_man$workers.geih)^2))
...
```
The MAE and RMSE Result show an improvement, as we can visually see but both of these models are not suitable for prediction in this case. We are going to analyze another more robust prediction models that might capture better the behavior of our variables and could produce more trustworthy predictions. In this case we are going to analyze ARIMA, SARIMA, and Random Forrest models. 
...
workers_ts <- ts(train_data_man$workers.geih, frequency = 12)
...
After differencing twice we can see that our p-value is under the threshold of 0.05, so we can assume that it is stationary and proceed to predict our variable.  
...
```
*ACF*: The ACF plot shows the autocorrelation of the series with its lags. Ideally, for a time series that has no autocorrelation, we would expect to see the plot drop to near-zero quickly. In this case, after differencing twice, the ACF plot shows that the autocorrelations at most lags are within the blue dashed confidence intervals, implying that they are not significantly different from zero. 
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_man$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_man$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_man$workers.geih))

# Define the time series plot range
time_range <- range(manizales_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(manizales_dataset_subset$workers.geih, quantile(test_data_man$predicted_workers,0.25), test_data_man$predicted_workers_sig, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(manizales_dataset_subset$date, manizales_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in MANIZALES A.M.")
...
mae_arima <- mean(abs(test_data_man$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_man$workers.geih - forecasts$mean)^2))
...
ols_mae <- mean(abs(test_data_man$predicted_workers - test_data_man$workers.geih))
ols_rmse <- sqrt(mean((test_data_man$predicted_workers - test_data_man$workers.geih)^2))
ols_summary <- summary(stepwise_model_man)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 4.656e-08, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_man),
  AICc = AIC(stepwise_model_man) + (2 * length(coef(stepwise_model_man)) * (length(coef(stepwise_model_man)) + 1)) / (length(resid(stepwise_model_man)) - length(coef(stepwise_model_man)) - 1),
  BIC = BIC(stepwise_model_man)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_man$predicted_workers_sig - test_data_man$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_man$predicted_workers_sig - test_data_man$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_man_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 1.082e-08, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_man_sig),
  AICc = AIC(stepwise_model_man_sig) + (2 * length(coef(stepwise_model_man_sig)) * (length(coef(stepwise_model_man_sig)) + 1)) / (length(resid(stepwise_model_man_sig)) - length(coef(stepwise_model_man_sig)) - 1),
  BIC = BIC(stepwise_model_man_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_man$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_man$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_man$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_man$workers.geih - forecasts_d$mean)^2))
...
# Calculate residuals
residuals <- test_data_man$workers.geih - forecasts$mean
...
*Residuals*: Use to visually assess the randomness and distribution of the residuals in our prediction. The plot of the residuals from the ARIMA model should ideally show no discernible pattern if the model is well-fitted. However, the displayed trend of increasing residuals suggests that there could be some systematic variation in the data not captured by the model, possibly indicating a missing component such as trend or seasonality which should be accounted for.
*ACF*: Used to show the correlation of the time series with its own lagged values.  It shows that most of the lags are within the bounds, hinting at minimal autocorrelation.
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_man$workers.geih))
```
...
workers_range <- range(manizales_dataset_subset$workers.geih, 
                       quantile(test_data_man$predicted_workers,0.25), 
                       test_data_man$predicted_workers_sig, 
                       all_confidence_intervals)

plot(manizales_dataset_subset$date, manizales_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in MANIZALES A.M.")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_man$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_man$date, test_data_man$predicted_workers, col='red', lwd=1.8)
lines(test_data_man$date, test_data_man$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(test_data_man$date)), col="black", lwd=1.8, lty=2)
...
sarima_mae <- mean(abs(test_data_man$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_man$workers.geih - sarima_forecasts$mean)^2))
...
```{r Random forrest}
# Load data
data_rf <- manizales_dataset_subset
...
# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(manizales_dataset_subset$workers.geih, 180000,240000))

# Plot the actual data for the complete dataset
plot(manizales_dataset_subset$date, manizales_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=full_range,  main="Forecasts Comparison for Employed Workers in MANIZALES A.M.")
...
```{r}
# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(manizales_dataset_subset$workers.geih, 230000))
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_man)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_man_sig)
...
write_xlsx(model_summaries, path = "manizales_results.xlsx")
...
png("manizales_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
