...
Let's proceed to work on the prediction for the city of CÚCUTA A.M.
```{r}
cucuta_dataset <- employed_data %>%
  filter( employed_data$city == "CÚCUTA A.M.") %>%
...
```{r}
cucuta_dataset$date <- as.Date(paste0(cucuta_dataset$date, "-01"), format = "%Y-%m-%d")
plot(cucuta_dataset$date, cucuta_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
cuc_basic_ols <- lm(workers.geih ~ . - date, data = cucuta_dataset)
summary(cuc_basic_ols)
...
cucuta_dataset_subset <- cucuta_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(cucuta_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(cucuta_dataset_subset$workers.geih)
qqline(cucuta_dataset_subset$workers.geih, col = "red")
shapiro.test(cucuta_dataset_subset$workers.geih)
summary(cucuta_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = cucuta_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(cucuta_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
cucuta_dataset_subset$date <- as.Date(paste0(cucuta_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_cuc <- cucuta_dataset_subset[cucuta_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_cuc <- cucuta_dataset_subset[cucuta_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_cuc_ols <- lm(workers.geih ~ . - date, data = train_data_cuc)

# Summarize the model to look at coefficients
summary(model_cuc_ols)

```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 0.9174 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_cuc <- lm(workers.geih ~ . - date, data = train_data_cuc)
stepwise_model_cuc <- step(model_cuc, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_cuc)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_cuc$predicted_workers <- predict(stepwise_model_cuc, newdata = test_data_cuc)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cucuta_dataset_subset$workers.geih, mean(test_data_cuc$predicted_workers)))

# Plot the actual data with the adjusted y limits
plot(cucuta_dataset_subset$date, cucuta_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(cucuta_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_cuc$date, test_data_cuc$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_cuc))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_cuc), resid(stepwise_model_cuc), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_cuc)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_cuc)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_cuc$predicted_workers - test_data_cuc$workers.geih))
rmse <- sqrt(mean((test_data_cuc$predicted_workers - test_data_cuc$workers.geih)^2))
...
W = 0.96314, p-value = 0.3525: With a p-value greater than the conventional alpha level of 0.05, we do not have sufficient evidence to reject the null hypothesis that the residuals are normally distributed. This means the residuals can be considered normally distributed, and the assumption of normality is likely not violated for the OLS model.
...
DW = 2.3519, p-value = 0.02673: The Durbin-Watson statistic is moderately above 2, suggesting a potential for negative autocorrelation. However, a p-value below the 0.05 threshold suggests that there is statistically significant evidence of negative autocorrelation in the residuals, which could violate the independence assumption.
...
MAE: 104843.2 and RMSE: 130792.7: These metrics suggest that on average, the model’s predictions are off by a substantial margin from the actual values. Ideally, we would want both MAE and RMSE to be as low as possible. The high values here indicate that the model's predictive accuracy is limited
...
```{r}
coef_summary <- summary(stepwise_model_cuc)$coefficients
...
stepwise_model_cuc_sig <- lm(significant_formula, data = train_data_cuc)

# Summary of the new model
summary(stepwise_model_cuc_sig)
```

```{r}
# Predicting with the stepwise model
test_data_cuc$predicted_workers_sig <- predict(stepwise_model_cuc_sig, newdata = test_data_cuc)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cucuta_dataset_subset$workers.geih, mean(test_data_cuc$predicted_workers)))

# Plot the actual data for the complete dataset
plot(cucuta_dataset_subset$date, cucuta_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_cuc$date, test_data_cuc$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_cuc$date, test_data_cuc$predicted_workers_sig, col='gold')
...
xlim <- range(cucuta_dataset_subset$date)
ylim <- range(c(cucuta_dataset_subset$workers.geih, test_data_cuc$predicted_workers, test_data_cuc$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_cuc_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_cuc_sig), resid(stepwise_model_cuc_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_cuc_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_cuc_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_cuc$predicted_workers_sig - test_data_cuc$workers.geih))
rmse_sig <- sqrt(mean((test_data_cuc$predicted_workers_sig - test_data_cuc$workers.geih)^2))
...
```
The MAE and RMSE Result show an improvement, as we can see but the prediction is still not optimal at all. We are going to analyze another more robust prediction models that might capture better the behavior of our variables and could produce more trustworthy predictions. In this case we are going to analyze ARIMA, SARIMA, and Random Forrest models. 
...
```
After differencing we can see that our p-value is below threshold of 0.05, so we can assume that it is stationary and proceed to predict our variable.  
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_cuc$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_cuc$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_cuc$workers.geih))

# Define the time series plot range
time_range <- range(cucuta_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(cucuta_dataset_subset$workers.geih, quantile(test_data_cuc$predicted_workers,0.25), test_data_cuc$predicted_workers_sig, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(cucuta_dataset_subset$date, cucuta_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in CUCUTA")
...
mae_arima <- mean(abs(test_data_cuc$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_cuc$workers.geih - forecasts$mean)^2))
...
ols_mae <- mean(abs(test_data_cuc$predicted_workers - test_data_cuc$workers.geih))
ols_rmse <- sqrt(mean((test_data_cuc$predicted_workers - test_data_cuc$workers.geih)^2))
ols_summary <- summary(stepwise_model_cuc)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 1.77e-05, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_cuc),
  AICc = AIC(stepwise_model_cuc) + (2 * length(coef(stepwise_model_cuc)) * (length(coef(stepwise_model_cuc)) + 1)) / (length(resid(stepwise_model_cuc)) - length(coef(stepwise_model_cuc)) - 1),
  BIC = BIC(stepwise_model_cuc)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_cuc$predicted_workers_sig - test_data_cuc$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_cuc$predicted_workers_sig - test_data_cuc$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_cuc_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 0.0001939, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_cuc_sig),
  AICc = AIC(stepwise_model_cuc_sig) + (2 * length(coef(stepwise_model_cuc_sig)) * (length(coef(stepwise_model_cuc_sig)) + 1)) / (length(resid(stepwise_model_cuc_sig)) - length(coef(stepwise_model_cuc_sig)) - 1),
  BIC = BIC(stepwise_model_cuc_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_cuc$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_cuc$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_cuc$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_cuc$workers.geih - forecasts_d$mean)^2))
...
```
As we can see, the ARIMA models have better performances than the other models, but they might not be the most optimal ones. Let's see how the residuals of our auto ARIMA prediction, as well as our ACF and PACF plots look like in order to determine our next steps.

```{r Residual and ACF / PACF Analysis of ARIMA}
# Calculate residuals
residuals <- test_data_cuc$workers.geih - forecasts$mean
...
*PACF*: Used for identifying the number of AR (Autoregressive) terms. All of the spikes fall within the blue confidence limits, which indicates that autocorrelation is not significant.
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_cuc$workers.geih))
```
...
workers_range <- range(cucuta_dataset_subset$workers.geih, 
                       test_data_cuc$predicted_workers, 
                       test_data_cuc$predicted_workers_sig, 
                       all_confidence_intervals)

plot(cucuta_dataset_subset$date, cucuta_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in CUCUTA A.M.")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_cuc$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_cuc$date, test_data_cuc$predicted_workers, col='red', lwd=1.8)
lines(test_data_cuc$date, test_data_cuc$predicted_workers_sig, col='gold', lwd=1.8)
...
# Plot the actual data for the complete dataset
plot(cucuta_dataset_subset$date, cucuta_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in CUCUTA A.M.")
...
abline(v = as.numeric(min(test_data_cuc$date)), col="black", lwd=1.8, lty=2)
...
sarima_mae <- mean(abs(test_data_cuc$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_cuc$workers.geih - sarima_forecasts$mean)^2))
...
print(model_summaries)
```
The SARIMA model gave us a better result in the Residual_SE, AIC, AICc, and BIC than the previous models. Let's see if we can obtain better results fitting a Random Forrest Model to see if it can outperform the models we have so far.
```{r Random forrest}
# Load data
data_rf <- cucuta_dataset_subset
...
# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cucuta_dataset_subset$workers.geih, 450000))

# Plot the actual data for the complete dataset
plot(cucuta_dataset_subset$date, cucuta_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=full_range,  main="Forecasts Comparison for Employed Workers in CUCUTA A.M.")
...
legend("topleft", inset=c(0, 0), # Negative inset x moves legend to the right
       legend=c("Actual (2016-2019)", "OLS Prediction (2019-07 onward)", 
                "OLS Prediction (2019-07 onward) Sig. Variables", "ARIMA Forecast","ARIMA d=3 Forecast", 
                "SARIMA Forecast", "RF Predictio","RF Prediction (Best RMSE)", "RF Prediction (Best R-squared)"), 
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_cuc)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_cuc_sig)
...
write_xlsx(model_summaries, path = "cucuta_results.xlsx")
...
png("cucuta_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
