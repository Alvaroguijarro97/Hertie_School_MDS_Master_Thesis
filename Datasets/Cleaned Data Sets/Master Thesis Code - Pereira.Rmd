...
Let's proceed to work on the prediction for the city of PEREIRA A.M.
```{r}
pereira_dataset <- employed_data %>%
  filter( employed_data$city == "PEREIRA A.M.") %>%
...
```{r}
pereira_dataset$date <- as.Date(paste0(pereira_dataset$date, "-01"), format = "%Y-%m-%d")
plot(pereira_dataset$date, pereira_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
per_basic_ols <- lm(workers.geih ~ . - date, data = pereira_dataset)
summary(per_basic_ols)
...
pereira_dataset_subset <- pereira_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(pereira_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(pereira_dataset_subset$workers.geih)
qqline(pereira_dataset_subset$workers.geih, col = "red")
shapiro.test(pereira_dataset_subset$workers.geih)
summary(pereira_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = pereira_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(pereira_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
pereira_dataset_subset$date <- as.Date(paste0(pereira_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_per <- pereira_dataset_subset[pereira_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_per <- pereira_dataset_subset[pereira_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_per <- lm(workers.geih ~ . - date, data = train_data_per)

# Summarize the model to look at coefficients
summary(model_per)

```
We have too many variables right now for out model, and it is not statistically significant (p value < 0.05), the high R-square of 0.9252 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_per <- lm(workers.geih ~ . - date, data = train_data_per)
stepwise_model_per <- step(model_per, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_per)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_per$predicted_workers <- predict(stepwise_model_per, newdata = test_data_per)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(pereira_dataset_subset$workers.geih, quantile(test_data_per$predicted_workers,0.25), 0))

# Plot the actual data with the adjusted y limits
plot(pereira_dataset_subset$date, pereira_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(pereira_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_per$date, test_data_per$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_per))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_per), resid(stepwise_model_per), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_per)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_per)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_per$predicted_workers - test_data_per$workers.geih))
rmse <- sqrt(mean((test_data_per$predicted_workers - test_data_per$workers.geih)^2))
...
W = 0.90353, p-value = 0.008838: The p-value is smaller than 0.05, so we reject the null hypothesis of normality. This indicates that the residuals of the model can not be considered normally distributed, which dissatisfies one of the OLS assumptions.
*Homoscedasticity (Plot of Residuals vs. Fitted Values)*
The residuals plot does not show a random dispersion of points around the horizontal line at zero, which suggests that there might be problems with homoscedasticity. It is important to note the potential clustering of data points around the center, and the funnel shape by the end. 

*Independence (Durbin-Watson test)*
DDW = 3.5145, p-value = 0.9913: A Durbin-Watson statistic over 2 suggests that there is autocorrelation in the residuals, which meets the assumption of independence. However, the p-value here is over 0.05, indicating some evidence of negative autocorrelation. This could be problematic for the validity of the OLS estimates.
*Multicollinearity (VIF)*
The reported VIF values are high, indicating potential multicollinearity among the predictors. Particularly, variables with VIF scores in the hundreds or thousands far exceed common thresholds of 5 or 10, suggesting that multicollinearity might be inflating the variances of the parameter estimates and could be problematic for the model.
*Prediction Performance (MAE and RMSE)*
MAE: 1150147 and RMSE: 1931806  suggest that the model's predictions deviate extremely from the actual values by a large margin. Lower values of MAE and RMSE would indicate better predictive accuracy. The large values here indicate the model may not predict the target variable very well.
...
```{r}
coef_summary <- summary(stepwise_model_per)$coefficients
...
stepwise_model_per_sig <- lm(significant_formula, data = train_data_per)

# Summary of the new model
summary(stepwise_model_per_sig)

```
```{r}
# Predicting with the stepwise model
test_data_per$predicted_workers_sig <- predict(stepwise_model_per_sig, newdata = test_data_per)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(pereira_dataset_subset$workers.geih, mean(test_data_per$predicted_workers)))

# Plot the actual data for the complete dataset
plot(pereira_dataset_subset$date, pereira_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_per$date, test_data_per$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_per$date, test_data_per$predicted_workers_sig, col='gold')
...
xlim <- range(pereira_dataset_subset$date)
ylim <- range(c(pereira_dataset_subset$workers.geih, test_data_per$predicted_workers, test_data_per$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_per_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_per_sig), resid(stepwise_model_per_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_per_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_per_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_per$predicted_workers_sig - test_data_per$workers.geih))
rmse_sig <- sqrt(mean((test_data_per$predicted_workers_sig - test_data_per$workers.geih)^2))
...
```
The MAE and RMSE Result did not show a significant improve, as we can visually see the model is not effectively predicting our workers variable. We are going to analyze another more robust prediction models that might capture better the behavior of our variables and could produce more trustworthy predictions. In this case we are going to analyze ARIMA, SARIMA, and Random Forrest models. 
...
workers_ts <- ts(train_data_per$workers.geih, frequency = 12)
...
acf(diff_workers_ts_2)
pacf(diff_workers_ts_2)

# Fitting an ARIMA model based on identified parameters
arima_model <- auto.arima(workers_ts, seasonal = FALSE, d=1) #Letting auto.arima find the best predictors
arima_model_d<- auto.arima(workers_ts, seasonal = FALSE, d=2) #setting the order of first-differencing to 1

summary(arima_model)
print("")
summary(arima_model_d)
```
*ACF*: The ACF plot shows the autocorrelation of the series with its lags. Ideally, for a time series that has no autocorrelation, we would expect to see the plot drop to near-zero quickly. In this case, after differencing twice, the ACF plot shows that most of the autocorrelations are within the confidence interval (the blue dotted lines), which is an indication that there may not be significant autocorrelation in the differenced series.
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_per$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_per$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_per$workers.geih))

# Define the time series plot range
time_range <- range(pereira_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(pereira_dataset_subset$workers.geih, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(pereira_dataset_subset$date, pereira_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in PEREIRA A.M.")
...
legend("topleft", inset=c(0, 0), # Negative inset x moves legend to the right
       legend=c("Actual (2016-2019)", "OLS Prediction (2019-07 onward)", 
                "OLS Prediction (2019-07 onward) Sig. Variables", "ARIMA Forecast", "ARIMA Forecast d=2",
...
mae_arima <- mean(abs(test_data_per$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_per$workers.geih - forecasts$mean)^2))
...
summary(forecasts)
```
We can visually confirm that the ARIMA model is predicting the variables better than the prior 2 models but both of the  ARIMA models are not capturing any seasonality trend of the data. Let's proceed to store the evaluation results of each to be able to better compare them at the end.
```{r}
...
ols_mae <- mean(abs(test_data_per$predicted_workers - test_data_per$workers.geih))
ols_rmse <- sqrt(mean((test_data_per$predicted_workers - test_data_per$workers.geih)^2))
ols_summary <- summary(stepwise_model_per)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 1.785e-05, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_per),
  AICc = AIC(stepwise_model_per) + (2 * length(coef(stepwise_model_per)) * (length(coef(stepwise_model_per)) + 1)) / (length(resid(stepwise_model_per)) - length(coef(stepwise_model_per)) - 1),
  BIC = BIC(stepwise_model_per)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_per$predicted_workers_sig - test_data_per$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_per$predicted_workers_sig - test_data_per$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_per_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 2.373e-05, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_per_sig),
  AICc = AIC(stepwise_model_per_sig) + (2 * length(coef(stepwise_model_per_sig)) * (length(coef(stepwise_model_per_sig)) + 1)) / (length(resid(stepwise_model_per_sig)) - length(coef(stepwise_model_per_sig)) - 1),
  BIC = BIC(stepwise_model_per_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_per$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_per$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_per$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_per$workers.geih - forecasts_d$mean)^2))
arima_d_summary <- summary(arima_model_d)

# Add ARIMA model metrics to the data frame
model_summaries <- rbind(model_summaries, data.frame(
  Model = "ARIMA with d=2",
  MAE = arima_d_mae,
  RMSE = arima_d_rmse,
  R_Squared = NA, # ARIMA models do not have R-squared
  P_Value = NA,   # P-value usually not directly applicable to ARIMA models
...
# Calculate residuals
residuals <- test_data_per$workers.geih - forecasts$mean
...
*Residuals*: Use to visually assess the randomness and distribution of the residuals in our prediction. The plot of the residuals of the ARIMA model appears to exhibit some pattern, as indicated by the valley shape under the zero line. This suggests that the model may not be fully capturing the underlying data structure, which might be due to seasonality or another form of autocorrelation not addressed by the model.
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_per$workers.geih))
```
...
workers_range <- range(pereira_dataset_subset$workers.geih)

plot(pereira_dataset_subset$date, pereira_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in PEREIRA A.M.")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_per$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_per$date, test_data_per$predicted_workers, col='red', lwd=1.8)
lines(test_data_per$date, test_data_per$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(test_data_per$date)), col="black", lwd=1.8, lty=2)

# Update the legend with SARIMA
legend("topleft", inset=c(0, 0), # Negative inset x moves legend to the right
       legend=c("Actual (2016-2019)", "ARIMA Forecast","ARIMA d=2 Forecast", 
                "SARIMA Forecast"), 
       col=c("black", "blue", "darkviolet","green"), 
       lty=c(1, 1, 1, 1), 
       lwd=c(2, 2, 2, 2), 
       merge=TRUE, 
       cex=0.7, 
       fill=c("black", "blue", "darkviolet","green"),
       bty="n") 
...
sarima_mae <- mean(abs(test_data_per$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_per$workers.geih - sarima_forecasts$mean)^2))
...
print(model_summaries)
```
The SARIMA model did not produce better results as the initial ARIMA predictions. Let's see if we can obtain better results fitting a Random Forrest Model to see if it can outperform the models we have so far.
```{r Random forrest}
# Load data
data_rf <- pereira_dataset_subset
...
legend("topleft", inset=c(0, 0), # Negative inset x moves legend to the right
       legend=c("Actual (2016-2019)", "OLS Prediction (2019-07 onward)", 
                "OLS Prediction (2019-07 onward) Sig. Variables", "ARIMA Forecast","ARIMA d=2 Forecast", 
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_per)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_per_sig)
...
write_xlsx(model_summaries, path = "pereira_results.xlsx")
...
png("pereira_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
