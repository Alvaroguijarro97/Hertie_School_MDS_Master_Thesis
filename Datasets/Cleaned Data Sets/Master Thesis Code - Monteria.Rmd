...
Let's proceed to work on the prediction for the city of MONTERÍA
```{r}
monteria_dataset <- employed_data %>%
  filter( employed_data$city == "MONTERÍA") %>%
...
```{r}
monteria_dataset$date <- as.Date(paste0(monteria_dataset$date, "-01"), format = "%Y-%m-%d")
plot(monteria_dataset$date, monteria_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
man_basic_ols <- lm(workers.geih ~ . - date, data = monteria_dataset)
summary(man_basic_ols)
...
monteria_dataset_subset <- monteria_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(monteria_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(monteria_dataset_subset$workers.geih)
qqline(monteria_dataset_subset$workers.geih, col = "red")
shapiro.test(monteria_dataset_subset$workers.geih)
summary(monteria_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = monteria_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(monteria_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
monteria_dataset_subset$date <- as.Date(paste0(monteria_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_mon <- monteria_dataset_subset[monteria_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_mon <- monteria_dataset_subset[monteria_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_mon <- lm(workers.geih ~ . - date, data = train_data_mon)

# Summarize the model to look at coefficients
summary(model_mon)

```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 0.8752 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_mon <- lm(workers.geih ~ . - date, data = train_data_mon)
stepwise_model_mon <- step(model_mon, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_mon)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_mon$predicted_workers <- predict(stepwise_model_mon, newdata = test_data_mon)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(monteria_dataset_subset$workers.geih, mean(test_data_mon$predicted_workers)))

# Plot the actual data with the adjusted y limits
plot(monteria_dataset_subset$date, monteria_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(monteria_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_mon$date, test_data_mon$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_mon))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_mon), resid(stepwise_model_mon), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_mon)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_mon)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_mon$predicted_workers - test_data_mon$workers.geih))
rmse <- sqrt(mean((test_data_mon$predicted_workers - test_data_mon$workers.geih)^2))
...
W = 0.94964, p-value = 0.1526: The test yields a p-value above the conventional 0.05 threshold, suggesting that the residuals may be considered normally distributed. There's no evidence to reject the null hypothesis of normality, and thus, this assumption holds for the OLS model residuals.
...
DW = 1.4913, p-value = 0.001553: The Durbin-Watson statistic is significantly less than 2, and the p-value is below 0.05, indicating evidence of positive autocorrelation among the residuals. This suggests a violation of the independence assumption, which may require further investigation or model refinement.
*Multicollinearity (VIF)*
The VIF values for some variables are extremely high, indicating potential multicollinearity issues. When VIF values are in the hundreds of millions or even larger, it signals that some independent variables are highly correlated with each other, which could be problematic because it can inflate the variance of the parameter estimates and may affect the stability of the model.
*Prediction Performance (MAE and RMSE)*
MAE: 22084.6 and RMSE: 27548.32: These metrics suggest that on average, the model’s predictions are off by a substantial margin from the actual values. Ideally, we would want both MAE and RMSE to be as low as possible. The high values here indicate that the model's predictive accuracy is limited.
...
```{r}
coef_summary <- summary(stepwise_model_mon)$coefficients
...
stepwise_model_mon_sig <- lm(significant_formula, data = train_data_mon)

# Summary of the new model
summary(stepwise_model_mon_sig)
```

```{r}
# Predicting with the stepwise model
test_data_mon$predicted_workers_sig <- predict(stepwise_model_mon_sig, newdata = test_data_mon)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(monteria_dataset_subset$workers.geih, mean(test_data_mon$predicted_workers)))

# Plot the actual data for the complete dataset
plot(monteria_dataset_subset$date, monteria_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_mon$date, test_data_mon$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_mon$date, test_data_mon$predicted_workers_sig, col='gold')
...
xlim <- range(monteria_dataset_subset$date)
ylim <- range(c(monteria_dataset_subset$workers.geih, test_data_mon$predicted_workers, test_data_mon$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_mon_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_mon_sig), resid(stepwise_model_mon_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_mon_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_mon_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_mon$predicted_workers_sig - test_data_mon$workers.geih))
rmse_sig <- sqrt(mean((test_data_mon$predicted_workers_sig - test_data_mon$workers.geih)^2))
...
```
The MAE and RMSE Result have the same the same result, as we can visually see. We are going to analyze another more robust prediction models that might capture better the behavior of our variables and could produce more trustworthy predictions. In this case we are going to analyze ARIMA, SARIMA, and Random Forrest models. 
...
```
*ACF*: The ACF plot shows the autocorrelation of the series with its lags. Ideally, for a time series that has no autocorrelation, we would expect to see the plot drop to near-zero quickly. In this case, after differencing twice, most of the spikes fall within the confidence bounds, implying that there's little to no autocorrelation in the residuals at those lags.
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_mon$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_mon$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_mon$workers.geih))

# Define the time series plot range
time_range <- range(monteria_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(monteria_dataset_subset$workers.geih, quantile(test_data_mon$predicted_workers,0.25), test_data_mon$predicted_workers_sig, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(monteria_dataset_subset$date, monteria_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in MONTERIA")
...
mae_arima <- mean(abs(test_data_mon$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_mon$workers.geih - forecasts$mean)^2))
...
summary(forecasts)
```
We can visually confirm that the ARIMA models are predicting the variables better than one of the prior 2 models but the ARIMA models with the d argument manually set to 2 and the one determined by the auto.arima function are just producing relative straight lines. Let's proceed to store the evaluation results of each to be able to better compare them at the end.
```{r}
...
ols_mae <- mean(abs(test_data_mon$predicted_workers - test_data_mon$workers.geih))
ols_rmse <- sqrt(mean((test_data_mon$predicted_workers - test_data_mon$workers.geih)^2))
ols_summary <- summary(stepwise_model_mon)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 1.008e-07, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_mon),
  AICc = AIC(stepwise_model_mon) + (2 * length(coef(stepwise_model_mon)) * (length(coef(stepwise_model_mon)) + 1)) / (length(resid(stepwise_model_mon)) - length(coef(stepwise_model_mon)) - 1),
  BIC = BIC(stepwise_model_mon)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_mon$predicted_workers_sig - test_data_mon$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_mon$predicted_workers_sig - test_data_mon$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_mon_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  AIC = AIC(stepwise_model_mon_sig),
  AICc = AIC(stepwise_model_mon_sig) + (2 * length(coef(stepwise_model_mon_sig)) * (length(coef(stepwise_model_mon_sig)) + 1)) / (length(resid(stepwise_model_mon_sig)) - length(coef(stepwise_model_mon_sig)) - 1),
  BIC = BIC(stepwise_model_mon_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_mon$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_mon$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_mon$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_mon$workers.geih - forecasts_d$mean)^2))
...
```
As we can see, the ARIMA models have a better performance than the other models, but they might still not be the most optimal one. Let's see how the residuals of our auto ARIMA prediction, as well as our ACF and PACF plots look like in order to determine our next steps.

```{r Residual and ACF / PACF Analysis of ARIMA}
# Calculate residuals
residuals <- test_data_mon$workers.geih - forecasts$mean
...
*Residuals*: Use to visually assess the randomness and distribution of the residuals in our prediction. The plot of the residuals from the ARIMA model does not show a completely random pattern, which is evident from the clear systematic deviation from the zero line. This indicates that the model may not be adequately capturing all the patterns in the data, potentially leaving out trends or cyclical components.
*ACF*: Used to show the correlation of the time series with its own lagged values.  It shows that the majority of the lags are within the blue dashed confidence intervals, implying that there's minimal autocorrelation in the residuals
*PACF*: Used for identifying the number of AR (Autoregressive) terms. Most of the spikes in the PACF plot are within the confidence intervals, which indicates that additional AR terms may not significantly improve the model.
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_mon$workers.geih))
```
...
workers_range <- range(monteria_dataset_subset$workers.geih, 
                       test_data_mon$predicted_workers, 
                       test_data_mon$predicted_workers_sig, 
                       all_confidence_intervals)

plot(monteria_dataset_subset$date, monteria_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in MONTERIA")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_mon$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_mon$date, test_data_mon$predicted_workers, col='red', lwd=1.8)
lines(test_data_mon$date, test_data_mon$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(test_data_mon$date)), col="black", lwd=1.8, lty=2)
...
sarima_mae <- mean(abs(test_data_mon$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_mon$workers.geih - sarima_forecasts$mean)^2))
...
```{r Random forrest}
# Load data
data_rf <- monteria_dataset_subset
...
# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(monteria_dataset_subset$workers.geih))

# Plot the actual data for the complete dataset
plot(monteria_dataset_subset$date, monteria_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=full_range,  main="Forecasts Comparison for Employed Workers in MONTERIA")
...
```{r}
# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(monteria_dataset_subset$workers.geih, 165000))

# Plot the actual data for the complete dataset
plot(monteria_dataset_subset$date, monteria_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=full_range, main="Forecasts Comparison for Employed Workers in MANIZALES")
...
# Random Forest model predictions
lines(test_dates, predictions_rf, col='darkgreen', lwd=1.8)
...
legend("topleft", inset=c(0, 0), # Negative inset x moves legend to the right
       ...
       col=c("black", "red", "gold", "blue", "darkviolet", "green", "darkgreen","orange", "cyan"), 
       lty=c(1, 1, 1, 1, 1, 1, 1, 2, 2), 
       lwd=c(2, 2, 2, 2, 2, 2, 2, 2, 2), 
       merge=TRUE, 
       cex=0.7, 
       fill=c("black", "red", "gold", "blue", "darkviolet", "green", "darkgreen","orange", "cyan"),
       bty="n") 
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_mon)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_mon_sig)
...
write_xlsx(model_summaries, path = "monteria_results.xlsx")
...
png("monteria_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
