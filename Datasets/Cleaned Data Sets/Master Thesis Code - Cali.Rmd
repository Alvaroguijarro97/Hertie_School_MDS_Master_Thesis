...
Let's proceed to work on the prediction for the city of CALI A.M.
```{r}
cali_dataset <- employed_data %>%
  filter( employed_data$city == "CALI  A.M.") %>%
...
```{r}
cali_dataset$date <- as.Date(paste0(cali_dataset$date, "-01"), format = "%Y-%m-%d")
plot(cali_dataset$date, cali_dataset$workers.geih, type='l', main='Evolution of workers during 2016-12 and 2019-12', xlab='Date', ylab='Number of Workers')
```
...
cal_basic_ols <- lm(workers.geih ~ . - date, data = cali_dataset)
summary(cal_basic_ols)
...
cali_dataset_subset <- cali_dataset[, selected_variables]

#Let's run some tests to see what is the distribution of our prediction variable
hist(cali_dataset_subset$workers.geih, main = "Histogram of Number of Workers", xlab = "Number of Workers", breaks = "Sturges")
qqnorm(cali_dataset_subset$workers.geih)
qqline(cali_dataset_subset$workers.geih, col = "red")
shapiro.test(cali_dataset_subset$workers.geih)
summary(cali_dataset_subset$workers.geih)
```
...
for (variable in variables) {
  ...
  model <- lm(formula, data = cali_dataset_subset)
  
  # Calculate R-squared value
  r_squared <- summary(model)$r.squared
  
  # Create the plot
  g <- ggplot(cali_dataset_subset, aes_string(x = variable, y = "workers.geih")) +
    geom_point() +
...
cali_dataset_subset$date <- as.Date(paste0(cali_dataset_subset$date, "-01"), format = "%Y-%m-%d")
train_data_cal <- cali_dataset_subset[cali_dataset_subset$date < as.Date("2019-07-01"), ]
test_data_cal <- cali_dataset_subset[cali_dataset_subset$date >= as.Date("2019-07-01"), ]

#let's fit a model
model_cal <- lm(workers.geih ~ . - date, data = train_data_cal)

# Summarize the model to look at coefficients
summary(model_cal)

```
We have too many variables right now for out model, even though it is statistically significant (p value < 0.05), the high R-square of 0.9885 suggest some sort of over-fitting. Let's do a stepwise regression for our model, in which we can automatically select the most important variables for this model. 

```{r}
model_cal <- lm(workers.geih ~ . - date, data = train_data_cal)
stepwise_model_cal <- step(model_cal, direction = "both", trace = FALSE)

# Print the summary of the final model
summary(stepwise_model_cal)
```
Let's evaluate how this model performed.
```{r}
# Predicting with the stepwise model
test_data_cal$predicted_workers <- predict(stepwise_model_cal, newdata = test_data_cal)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cali_dataset_subset$workers.geih, mean(test_data_cal$predicted_workers)))

# Plot the actual data with the adjusted y limits
plot(cali_dataset_subset$date, cali_dataset_subset$workers.geih, type='l', col='black',
     xlab='Date', ylab='Number of Workers', xlim=range(cali_dataset_subset$date), ylim=full_range, 
     main='Evolution of workers during 2016-12 and 2019-12')

# Add the OLS predictions to the plot
lines(test_data_cal$date, test_data_cal$predicted_workers, col='red')
...
shapiro.test(resid(stepwise_model_cal))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_cal), resid(stepwise_model_cal), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_cal)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_cal)  # VIF > 5 or 10 might be problematic

# Performance metrics
mae <- mean(abs(test_data_cal$predicted_workers - test_data_cal$workers.geih))
rmse <- sqrt(mean((test_data_cal$predicted_workers - test_data_cal$workers.geih)^2))
...
W = 0.96876, p-value = 0.4855: The p-value is well above the conventional threshold of 0.05, suggesting that there is no significant departure from normality in the residuals. This indicates that the normality assumption is satisfied for this OLS model.

*Homoscedasticity (Plot of Residuals vs. Fitted Values)*
The residuals appear to show a random scatter of residuals around the horizontal line at zero, without any apparent pattern. This randomness in the dispersion is indicative of constant variance in the errors, or homoscedasticity.

*Independence (Durbin-Watson test)*
DW = 3.2156, p-value = 0.6779: The Durbin-Watson statistic is notably above 2, implying negative autocorrelation. Nevertheless, since the p-value is significantly greater than 0.05, there is no strong evidence of autocorrelation, meaning the independence assumption is likely not violated.

*Multicollinearity (VIF)*
The VIF values are substantially high for several variables, indicating that multicollinearity is likely an issue. High VIF values suggest that some predictors have a strong linear relationship with other predictors, which can affect the reliability of the coefficient estimates and the model's overall interpretability.

*Prediction Performance (MAE and RMSE)*
MAE: 189383.5 and RMSE: 267407.5: These metrics suggest that on average, the model’s predictions are off by a substantial margin from the actual values. Ideally, we would want both MAE and RMSE to be as low as possible. The high values here indicate that the model's predictive accuracy is limited.
...
```{r}
coef_summary <- summary(stepwise_model_cal)$coefficients
...
significant_formula <- as.formula(paste("workers.geih ~", paste(significant_vars[significant_vars != c("(Intercept)")], collapse = " + ")))

# Fit the new model with significant variables only
stepwise_model_cal_sig <- lm(significant_formula, data = train_data_cal)

# Summary of the new model
summary(stepwise_model_cal_sig)
```

```{r}
# Predicting with the stepwise model
test_data_cal$predicted_workers_sig <- predict(stepwise_model_cal_sig, newdata = test_data_cal)

# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cali_dataset_subset$workers.geih, mean(test_data_cal$predicted_workers)))

# Plot the actual data for the complete dataset
plot(cali_dataset_subset$date, cali_dataset_subset$workers.geih, type='l', col='black', xlab='Date', ylab='Number of Workers', ylim=full_range)

# Add the forecasted data starting from the prediction date
lines(test_data_cal$date, test_data_cal$predicted_workers, col='red')

# Add the forecasted data starting from the prediction date
lines(test_data_cal$date, test_data_cal$predicted_workers_sig, col='gold')
...
xlim <- range(cali_dataset_subset$date)
ylim <- range(c(cali_dataset_subset$workers.geih, test_data_cal$predicted_workers, test_data_cal$predicted_workers_sig))
...
```{r}
# Normality - Shapiro-Wilk normality test on model residuals
shapiro.test(resid(stepwise_model_cal_sig))

# Homoscedasticity - Plot residuals vs. fitted values
plot(fitted(stepwise_model_cal_sig), resid(stepwise_model_cal_sig), xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "red")

# Independence - Durbin-Watson test for autocorrelation
dwtest(stepwise_model_cal_sig)

# Multicollinearity - Variance Inflation Factor for multicollinearity
vif(stepwise_model_cal_sig)  # VIF > 5 or 10 might be problematic
...
mae_sig <- mean(abs(test_data_cal$predicted_workers_sig - test_data_cal$workers.geih))
rmse_sig <- sqrt(mean((test_data_cal$predicted_workers_sig - test_data_cal$workers.geih)^2))
...
```
The MAE and RMSE Result show an improvement, but the variables we've choosen so far are not predicting correctly our interest variable. We are going to analyze another more robust prediction models that might capture better the behavior of our variables and could produce more trustworthy predictions. In this case we are going to analyze ARIMA, SARIMA, and Random Forrest models. 
...
arima_model_d<- auto.arima(workers_ts, seasonal = FALSE, d=2) #setting the order of first-differencing to 2

summary(arima_model)
print("")
summary(arima_model_d)
```
*ACF*: The ACF plot shows the autocorrelation of the series with its lags. Ideally, for a time series that has no autocorrelation, we would expect to see the plot drop to near-zero quickly. In this case, after differencing twice, the ACF plot shows that most of the correlations are within the blue dashed confidence intervals, which implies that the autocorrelations at most lags are not statistically significant.

*PACF*: Here, we observe that most of the spikes fall within the confidence interval and are close to zero, which suggests that there may not be significant partial autocorrelations in the data.
...
arima_forecast_dates <- seq(from = as.Date("2019-07-01"), 
                            by = "month", 
                            length.out = length(test_data_cal$date))

# Forecasting
forecasts <- forecast(arima_model, h = length(test_data_cal$workers.geih))
forecasts_d <- forecast(arima_model_d, h= length(test_data_cal$workers.geih))

# Define the time series plot range
time_range <- range(cali_dataset_subset$date)

# Define the y-axis plot range to include all data
workers_range <- range(cali_dataset_subset$workers.geih, quantile(test_data_cal$predicted_workers,0.25), test_data_cal$predicted_workers_sig, forecasts$lower[,2], forecasts$upper[,2])

# Plot the actual data for the complete dataset
plot(cali_dataset_subset$date, cali_dataset_subset$workers.geih, type='l', col='black', 
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, main="Forecasts Comparison for Employed Workers in CALI A.M.")
...
mae_arima <- mean(abs(test_data_cal$workers.geih - forecasts$mean))
rmse_arima <- sqrt(mean((test_data_cal$workers.geih - forecasts$mean)^2))
...
summary(forecasts)
```
We can visually confirm that the ARIMA models are predicting the variables better than the prior OLS 2 models but the ARIMA models with the d argument manually set to 2 and the one determined by the auto.arima function are just producing somewhat of straight lines. Let's proceed to store the evaluation results of each to be able to better compare them at the end.
```{r}
...
ols_mae <- mean(abs(test_data_cal$predicted_workers - test_data_cal$workers.geih))
ols_rmse <- sqrt(mean((test_data_cal$predicted_workers - test_data_cal$workers.geih)^2))
ols_summary <- summary(stepwise_model_cal)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 1.139e-08, # Manually get it
  Residual_SE = ols_summary$sigma,
  AIC = AIC(stepwise_model_cal),
  AICc = AIC(stepwise_model_cal) + (2 * length(coef(stepwise_model_cal)) * (length(coef(stepwise_model_cal)) + 1)) / (length(resid(stepwise_model_cal)) - length(coef(stepwise_model_cal)) - 1),
  BIC = BIC(stepwise_model_cal)
))

# Calculate metrics for the OLS model with significant variables
sig_ols_mae <- mean(abs(test_data_cal$predicted_workers_sig - test_data_cal$workers.geih))
sig_ols_rmse <- sqrt(mean((test_data_cal$predicted_workers_sig - test_data_cal$workers.geih)^2))
sig_ols_summary <- summary(stepwise_model_cal_sig)
...
model_summaries <- rbind(model_summaries, data.frame(
  ...
  P_Value = 5.067e-10, # Manually get it
  Residual_SE = sig_ols_summary$sigma,
  AIC = AIC(stepwise_model_cal_sig),
  AICc = AIC(stepwise_model_cal_sig) + (2 * length(coef(stepwise_model_cal_sig)) * (length(coef(stepwise_model_cal_sig)) + 1)) / (length(resid(stepwise_model_cal_sig)) - length(coef(stepwise_model_cal_sig)) - 1),
  BIC = BIC(stepwise_model_cal_sig)
))

# Calculate metrics for ARIMA model
arima_mae <- mean(abs(test_data_cal$workers.geih - forecasts$mean))
arima_rmse <- sqrt(mean((test_data_cal$workers.geih - forecasts$mean)^2))
...
# Calculate metrics for ARIMA model
arima_d_mae <- mean(abs(test_data_cal$workers.geih - forecasts_d$mean))
arima_d_rmse <- sqrt(mean((test_data_cal$workers.geih - forecasts_d$mean)^2))
...
# Calculate residuals
residuals <- test_data_cal$workers.geih - forecasts$mean
...
*Residuals*: Use to visually assess the randomness and distribution of the residuals in our prediction. The visible upward trend in the residual plot suggests that the model may have missed some aspects of the underlying data structure, potentially a trend or seasonality that hasn't been accounted for in the model.
*ACF*: Used to show the correlation of the time series with its own lagged values.  It mostly shows autocorrelations within the confidence bounds, suggesting that the model has captured most of the autocorrelation in the data. However, there may still be some autocorrelation present, as indicated by any bars that extend beyond these bounds.
...
sarima_forecasts <- forecast(sarima_model, h = length(test_data_cal$workers.geih))
```
...
workers_range <- range(cali_dataset_subset$workers.geih, 
                       quantile(test_data_cal$predicted_workers,0.25), 
                       quantile(test_data_cal$predicted_workers_sig,0.25), 
                       all_confidence_intervals)

plot(cali_dataset_subset$date, cali_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=workers_range, 
     main="Forecasts Comparison for Employed Workers in CALI A.M.")

# ARIMA forecast line
lines(arima_forecast_dates, forecasts$mean, col='blue', lwd=1.8)

# SARIMA forecast line
sarima_forecast_dates <- seq(from = min(test_data_cal$date), 
                             by = "month", 
...
# OLS forecasted data
lines(test_data_cal$date, test_data_cal$predicted_workers, col='red', lwd=1.8)
lines(test_data_cal$date, test_data_cal$predicted_workers_sig, col='gold', lwd=1.8)
...
abline(v = as.numeric(min(test_data_cal$date)), col="black", lwd=1.8, lty=2)
...
sarima_mae <- mean(abs(test_data_cal$workers.geih - sarima_forecasts$mean))
sarima_rmse <- sqrt(mean((test_data_cal$workers.geih - sarima_forecasts$mean)^2))
...
print(model_summaries)
```
The SARIMA model showed an improvement over the initial ARIMA predictions. Let's see if we can obtain better results fitting a Random Forrest Model to see if it can outperform the models we have so far.
```{r Random forrest}
# Load data
data_rf <- cali_dataset_subset
...
# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cali_dataset_subset$workers.geih, 1100000))

# Plot the actual data for the complete dataset
plot(cali_dataset_subset$date, cali_dataset_subset$workers.geih, type='l', col='black', lwd=1.8,
     xlab='Date', ylab='Number of Workers', xlim=time_range, ylim=full_range,  main="Forecasts Comparison for Employed Workers in CALI A.M.")
...
```{r}
# Find the full range of values for workers including both actual values and predictions
full_range <- range(c(cali_dataset_subset$workers.geih, 1000000))
...
# For OLS models
model_var_summary$OLS <- extract_lm_vars(stepwise_model_cal)
model_var_summary$OLS_Significant <- extract_lm_vars(stepwise_model_cal_sig)
...
write_xlsx(model_summaries, path = "cali_results.xlsx")
...
png("cali_results.png", width=600, height=500)  # Adjust 'width' and 'height' as needed
